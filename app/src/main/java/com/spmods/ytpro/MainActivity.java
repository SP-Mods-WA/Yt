...
public class MainActivity extends Activity {
  ...
  // ✅ YouTube IFrame Player Variables (add after existing variables)
private RelativeLayout customYouTubePlayerContainer;
private WebView youtubeIframePlayer;
private ImageView btnYouTubePlayPause, btnPrevious, btnNext, btnCloseYouTube;
private SeekBar youtubeSeekBar;
private TextView youtubeTimeDisplay, youtubeVideoTitle;
private ProgressBar youtubeLoadingIndicator;
private Handler youtubeProgressHandler = new Handler();
...
private String currentVideoId = "";
  ...
  public void load(boolean dl) {
    ...
    if (url.contains("/feed/notifications")) {
        ...
        // ✅ Hide YouTube header & bottom nav
        web.evaluateJavascript(
            ...
            "    ytm-pivot-bar-renderer { display: none !important; }" +
            "    #masthead { display: none !important; }" +
            "    body { padding-top: 0px !important; padding-bottom: 65px !important; }" +
            "  ';" +
            "  document.head.appendChild(style);" +
            "})();",
            null
...
private String loadScriptFromAssets(String filename) {
  ...
  private void setupCustomHeader() {
    ...
    iconNotifications.setOnClickListener(v -> {
        userNavigated = true;
        web.loadUrl("https://m.youtube.com/feed/notifications");
    });
...
public void onBackPressed() {
    // ✅ Check if custom YouTube player is open
    if (customYouTubePlayerContainer != null && 
        customYouTubePlayerContainer.getVisibility() == View.VISIBLE) {
        closeYouTubePlayer();
  ...
  public class WebAppInterface {
    ...
    @JavascriptInterface
    public void playInYouTubePlayer(String videoId, String videoTitle) {
        Log.d("JSInterface", "📹 YouTube IFrame requested: " + videoTitle);
        loadYouTubeVideo(videoId, videoTitle);
    }
  }
  ...
  // ✅✅✅ YouTube IFrame Player Methods - මෙතනින් පටන් ගන්න ✅✅✅

private void initYouTubeIframePlayer() {
    customYouTubePlayerContainer = findViewById(R.id.customYouTubePlayerContainer);
    youtubeIframePlayer = findViewById(R.id.youtubeIframePlayer);
    btnYouTubePlayPause = findViewById(R.id.btnYouTubePlayPause);
    btnPrevious = findViewById(R.id.btnPrevious);
    btnNext = findViewById(R.id.btnNext);
    btnCloseYouTube = findViewById(R.id.btnCloseYouTube);
    youtubeSeekBar = findViewById(R.id.youtubeSeekBar);
    youtubeTimeDisplay = findViewById(R.id.youtubeTimeDisplay);
    youtubeVideoTitle = findViewById(R.id.youtubeVideoTitle);
    youtubeLoadingIndicator = findViewById(R.id.youtubeLoadingIndicator);
    
    // Setup WebView for IFrame
    WebSettings settings = youtubeIframePlayer.getSettings();
    settings.setJavaScriptEnabled(true);
    settings.setDomStorageEnabled(true);
    settings.setMediaPlaybackRequiresUserGesture(false);
    settings.setAllowFileAccess(true);
    ...
    youtubeIframePlayer.setBackgroundColor(Color.BLACK);
    youtubeIframePlayer.addJavascriptInterface(new YouTubePlayerInterface(), "AndroidPlayer");
    
    // Play/Pause Button
    btnYouTubePlayPause.setOnClickListener(v -> {
        if (isYouTubePlaying) {
            youtubeIframePlayer.evaluateJavascript("player.pauseVideo();", null);
        } else {
            youtubeIframePlayer.evaluateJavascript("player.playVideo();", null);
        }
    });
    
    // Previous Button
    btnPrevious.setOnClickListener(v -> {
        youtubeIframePlayer.evaluateJavascript("player.previousVideo();", null);
    });
    
    // Next Button
    btnNext.setOnClickListener(v -> {
        youtubeIframePlayer.evaluateJavascript("player.nextVideo();", null);
    });
    
    // Close Button
    btnCloseYouTube.setOnClickListener(v -> {
        closeYouTubePlayer();
    });
    
    // SeekBar
    youtubeSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
        @Override
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser) {
                youtubeIframePlayer.evaluateJavascript(
    ...
    Log.d("YouTubePlayer", "✅ IFrame Player initialized");
}

private void loadYouTubeVideo(String videoId, String videoTitle) {
    runOnUiThread(() -> {
        ...
        // Show custom player
        customYouTubePlayerContainer.setVisibility(View.VISIBLE);
        customYouTubePlayerContainer.bringToFront();
        
        // Set title
        youtubeVideoTitle.setText(videoTitle);
        
        // Show loading
        youtubeLoadingIndicator.setVisibility(View.VISIBLE);
        
        // Load IFrame Player HTML
        String html = getYouTubeIframeHTML(videoId);
        youtubeIframePlayer.loadDataWithBaseURL(
...
private String getYouTubeIframeHTML(String videoId) {
    return "<!DOCTYPE html>" +
           "<html>" +
           "<head>" +
           ...
           "  AndroidPlayer.onStateChange(state);" +
           "  " +
           ...
           "    AndroidPlayer.updateProgress(current, duration);" +
...
public class YouTubePlayerInterface {
    ...
    public void onPlayerReady() {
        runOnUiThread(() -> {
            youtubeLoadingIndicator.setVisibility(View.GONE);
            Log.d("YouTubePlayer", "✅ Player ready");
        });
    }
        ...
        runOnUiThread(() -> {
            switch (state) {
                case 1: // PLAYING
                    ...
                    btnYouTubePlayPause.setImageResource(android.R.drawable.ic_media_pause);
                    youtubeLoadingIndicator.setVisibility(View.GONE);
                    Log.d("YouTubePlayer", "▶️ Playing");
                    break;
                case 2: // PAUSED
                    isYouTubePlaying = false;
                    btnYouTubePlayPause.setImageResource(android.R.drawable.ic_media_play);
                    Log.d("YouTubePlayer", "⏸️ Paused");
                    break;
                case 0: // ENDED
                    isYouTubePlaying = false;
                    btnYouTubePlayPause.setImageResource(android.R.drawable.ic_media_play);
                    Log.d("YouTubePlayer", "⏹️ Ended");
                    closeYouTubePlayer();
                    break;
                case 3: // BUFFERING
                    youtubeLoadingIndicator.setVisibility(View.VISIBLE);
    ...
    public void updateProgress(int current, int duration) {
        runOnUiThread(() -> {
            youtubeSeekBar.setMax(duration);
            youtubeSeekBar.setProgress(current);
            
            String currentTime = formatTime(current);
            String durationTime = formatTime(duration);
            youtubeTimeDisplay.setText(currentTime + " / " + durationTime);
        });
    }
}
...
private void closeYouTubePlayer() {
    runOnUiThread(() -> {
        ...
        customYouTubePlayerContainer.setVisibility(View.GONE);
        web.setVisibility(View.VISIBLE);
        isYouTubePlaying = false;
        youtubeProgressHandler.removeCallbacksAndMessages(null);
        Log.d("YouTubePlayer", "❌ Player closed");
    });
}

// ✅✅✅ YouTube IFrame Player Methods - මෙතන අවසන් ✅✅✅
