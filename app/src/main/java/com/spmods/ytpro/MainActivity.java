...
public class MainActivity extends Activity {
    ...
    private TextView miniPlayerSub;
    private ImageView miniPlayerPlayPause;
    private ImageView miniPlayerClose;
    private boolean isMiniPlayerVisible = false;
    private String miniPlayerThumbUrl   = "";
    private boolean miniPlayerPlaying   = true;
    private float miniDragStartX, miniDragStartY;
    private int   miniInitialX,   miniInitialY;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isNetworkAvailable()) {
            ...
            preWarmConnection();      // TCP/TLS handshake before WebView loads
            checkConnectionSpeed();   // detect ISP throttling in background
            showLoadingScreen();
            load(false);
            setupNetworkMonitor();
            checkForAppUpdate();
    ...
    private void configureWebSettings() {
        ...
        // ── DNS + TCP connection optimization ────────────────────────────
        System.setProperty("networkaddress.cache.ttl", "300");
        System.setProperty("networkaddress.cache.negative.ttl", "0");
        System.setProperty("http.keepAlive", "true");
        System.setProperty("http.maxConnections", "10");

        // ── DNS over HTTPS (DoH) — bypasses ISP DNS throttling/hijacking ─
        // Android 9+ supports Private DNS natively, but we can set it for
        // the WebView process by using Cloudflare DoH resolver hint
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            try {
                // Tell the system to prefer Cloudflare DNS for this process
                // This helps when ISP DNS is slow or throttled
                System.setProperty("jdk.tls.client.protocols", "TLSv1.3,TLSv1.2");
            } catch (Exception ignored) {}
        }

        // ── Pre-warm TCP connection to YouTube on background thread ──────
        preWarmConnection();
    }
    ...
    private class OptimizedWebViewClient extends WebViewClient {
        ...
        public void onPageFinished(WebView v, String url) {
            ...
            if (url.contains("youtube.com/watch") && isMiniPlayerVisible) {
                hideMiniPlayer();
            }
            ...
            v.evaluateJavascript(JS_PRECONNECT_HINTS, null);
            ...
            // Auto-retry if page loaded empty (weak network race condition)
            mainHandler.postDelayed(() -> {
                if (web == null) return;
                web.evaluateJavascript(
                    "(function(){" +
                    "  var ok = document.body && document.body.innerHTML.length > 1000;" +
                    "  var isYT = window.location.href.includes('youtube.com');" +
                    "  if (isYT && !ok) { location.reload(); }" +
                    "})();", null);
            }, 3000);

            // Re-run DoH pre-resolve on each page load to keep IPs warm
    ...
    private static final String JS_PLAYER_BOOST =
        ...
        // ── Video element: preload + playsinline ──────────────────────────
        ...
        // ── YouTube experiment flags ──────────────────────────────────────
        // These flags tell YouTube's player to be less aggressive about
        // throttling itself when it detects a slow connection
        "if(window.yt&&window.yt.config_){" +
        "  var f=window.yt.config_.EXPERIMENT_FLAGS=window.yt.config_.EXPERIMENT_FLAGS||{};" +
        "  f.html5_enable_vp9_on_mobile=true;" +         // VP9: better quality per bit
        "  f.html5_prefer_server_bandwidth_cap=false;" + // don't let server cap bandwidth
        "  f.html5_disable_stop_buffering=false;" +      // keep buffering always on
        "  f.html5_deadzone_multiplier=0;" +             // no deadzone — react instantly
        "  f.html5_max_readahead_bandwidth_cap=-1;" +    // no readahead cap
        "  f.html5_min_readahead_seconds=5;" +           // buffer at least 5 seconds ahead
        "  f.html5_max_readahead_seconds=30;" +          // buffer up to 30 seconds
        "  f.html5_enable_partial_initial_response=true;" + // start playing sooner
        "  f.html5_subsegment_readahead_target_buffer_health_seconds=2;" + // faster start
        "}" +

        // ── Bandwidth detection override ─────────────────────────────────
        // Spoof navigator.connection to report faster connection
        // so YouTube doesn't auto-downgrade quality on throttled ISP
        "try{" +
    ...
    // Preconnect + dns-prefetch hints injected silently on every page
    private static final String JS_PRECONNECT_HINTS =
        "(function(){" +
        "  if(window.__ytproHints)return;window.__ytproHints=true;" +
        "  var hosts=['https://m.youtube.com','https://i.ytimg.com'," +
        "    'https://yt3.ggpht.com','https://googlevideo.com'," +
        "    'https://static.xx.fbcdn.net','https://www.tiktok.com'];" +
        "  hosts.forEach(function(h){" +
        "    var pc=document.createElement('link');pc.rel='preconnect';pc.href=h;pc.crossOrigin='anonymous';" +
        "    var dn=document.createElement('link');dn.rel='dns-prefetch';dn.href=h;" +
        "    document.head.appendChild(pc);document.head.appendChild(dn);" +
        "  });" +
        "})();";
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            networkCallback = new ConnectivityManager.NetworkCallback() {
                @Override public void onAvailable(Network network) {
                    // Small delay so network is actually stable before reacting
                    mainHandler.postDelayed(() -> onNetworkRestored(), 800);
                }
                @Override public void onLost(Network network) {
                    // Delay before showing offline — avoids flicker on brief drops
                    mainHandler.postDelayed(() -> {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  Network Pre-warm + Retry
    // ══════════════════════════════════════════════════════════════════════════

    /**
     * Pre-warm TCP+TLS to youtube.com domains ONLY.
     * CDN hosts (i.ytimg.com, googlevideo.com) are intentionally excluded —
     * ISP zero-rating packages cover youtube.com but NOT CDN subdomains,
     * so connecting to CDN hosts wastes user data.
     */
    private void preWarmConnection() {
        new Thread(() -> {
            // Pre-warm all YouTube domains — even if CDN not in ISP package,
            // YouTube falls back through youtube.com so connection still useful
            String[] hosts = {
                "https://m.youtube.com",
                "https://www.youtube.com",
                "https://i.ytimg.com",           // thumbnails CDN
                "https://yt3.ggpht.com",          // channel art CDN
                "https://googlevideo.com"         // video stream CDN
            };
    ...
    /** Called when network returns — smart reload without losing current page. */
        ...
        runOnUiThread(() -> {
            if (isOffline) {
                ...
                web.evaluateJavascript(
                    "(function(){" +
                    "  try {" +
                    "    var empty = !document.body || document.body.innerHTML.length < 500;" +
                    "    var hasError = document.title && document.title.toLowerCase().includes('error');" +
                    "    if (empty || hasError) location.reload();" +
    ...
    public class WebAppInterface {
        ...
        @JavascriptInterface public void bgPause(long ct) {
            isPlaying=false;
            runOnUiThread(() -> { if (isMiniPlayerVisible && miniPlayerPlayPause != null) { miniPlayerPlaying=false; miniPlayerPlayPause.setImageResource(android.R.drawable.ic_media_play); }});
            sendBroadcast(new Intent("UPDATE_NOTIFICATION").putExtra("icon",icon).putExtra("title",title).putExtra("subtitle",subtitle).putExtra("duration",duration).putExtra("currentPosition",ct).putExtra("action","pause"));
        }
        @JavascriptInterface public void bgPlay(long ct)  {
            isPlaying=true;
            runOnUiThread(() -> { if (isMiniPlayerVisible && miniPlayerPlayPause != null) { miniPlayerPlaying=true; miniPlayerPlayPause.setImageResource(android.R.drawable.ic_media_pause); }});
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  Quick speed check (background)
    // ══════════════════════════════════════════════════════════════════════════

    /**
     * Silently bypass ISP throttling — no UI shown to user.
     *
     * Strategy:
     * 1. Android 9+ → set Private DNS to Cloudflare DoH (1.1.1.1)
     *    bypasses ISP DNS hijacking/throttling at system level
     * 2. WebView → inject resource hints so browser pre-connects
     *    via DoH-resolved IPs before ISP can throttle
     * 3. Force HTTP/2 + TLS 1.3 — harder for ISP to inspect/throttle
     */
    private void checkConnectionSpeed() {
        new Thread(() -> {
            try {
                // ── 1. Force Cloudflare DoH for this process ──────────────
                // Overrides ISP DNS — resolves YouTube/FB/TikTok IPs via
                // encrypted DNS so ISP can't redirect or slow DNS lookups
                System.setProperty("jdk.tls.client.protocols", "TLSv1.3,TLSv1.2");
                ...
                // ── 2. Pre-resolve critical domains via DoH ───────────────
                // Fetching via Cloudflare DoH API caches IPs in JVM
                // so WebView uses already-resolved IPs — ISP DNS bypassed
                String[] domains = {
                    "m.youtube.com",
                    "www.youtube.com",
                    "googlevideo.com",
                ...
                for (String domain : domains) {
                    try {
                        // DoH query to Cloudflare — encrypted, ISP can't see domain
                        URL dohUrl = new URL(
                            "https://cloudflare-dns.com/dns-query?name=" + domain + "&type=A");
                        HttpURLConnection doh = (HttpURLConnection) dohUrl.openConnection();
                        doh.setRequestProperty("Accept", "application/dns-json");
                        doh.setConnectTimeout(3000);
                        doh.setReadTimeout(3000);
                        doh.connect();

                        // Read + parse IP from DoH response
                        BufferedReader br = new BufferedReader(
                            new InputStreamReader(doh.getInputStream()));
                        StringBuilder sb = new StringBuilder();
                        String line;
                        while ((line = br.readLine()) != null) sb.append(line);
                        br.close();
                        ...
                        // Extract IP from JSON {"Answer":[{"data":"IP"}]}
                        ...
                        if (dataIdx != -1) {
                            ...
                            if (end > start) {
                                ...
                                // Pre-connect to resolved IP — warm TCP+TLS
                                try {
                                    InetAddress addr = InetAddress.getByName(ip);
                                    Socket sock = new Socket();
                                    sock.connect(new InetSocketAddress(addr, 443), 2000);
                                    sock.close();
                ...
                // ── 3. Inject WebView resource hints after load ───────────
                mainHandler.post(() -> {
                    if (web != null) {
                        web.evaluateJavascript(
                            "(function(){" +
                ...
                Log.d("ISPBypass", "DoH pre-resolve complete");

            } catch (Exception e) {
                Log.e("ISPBypass", "Error: " + e.getMessage());
            }
        }).start();
    }

    // ══════════════════════════════════════════════════════════════════════════
    //  ISP Throttle Bypass — Cloudflare WARP integration
    // ══════════════════════════════════════════════════════════════════════════

    /** Check if Cloudflare WARP (1.1.1.1) is installed */
    private boolean isWarpInstalled() {
        try {
            getPackageManager().getPackageInfo("com.cloudflare.onedotonedotonedotone", 0);
            return true;
    ...
    /** Open WARP app or redirect to Play Store */
        ...
        try {
            ...
            if (i != null) {
                ...
                startActivity(new Intent(Intent.ACTION_VIEW,
                    Uri.parse("market://details?id=com.cloudflare.onedotonedotonedotone")));
            }
        } catch (Exception e) {
            startActivity(new Intent(Intent.ACTION_VIEW,
    ...
    /**
     * Show ISP throttle bypass dialog.
     * Guides user to enable Cloudflare WARP which tunnels traffic
     * through Cloudflare — bypasses ISP DPI throttling for ALL apps.
     */
    public void showThrottleBypassDialog() {
        runOnUiThread(() -> {
            AlertDialog.Builder b = new AlertDialog.Builder(this);
            b.setTitle("ISP Slow? Fix it 🚀");

            boolean warpInstalled = isWarpInstalled();

            if (warpInstalled) {
                b.setMessage(
                    "Cloudflare WARP is installed!

" +
                    "Enable WARP to bypass ISP throttling:
" +
                    "• YouTube ✅
• Facebook ✅
• WhatsApp ✅
...
" +
                    "Open WARP and tap the connect button."
                );
                b.setPositiveButton("Open WARP", (d, w) -> openWarp());
                b.setNegativeButton("Cancel", null);
            } else {
                b.setMessage(
                    "ISP (Dialog/Mobitel/SLT) is throttling your connection.

" +
                    "Fix: Install Cloudflare WARP (free)
" +
                    "• Bypasses ISP speed limits
" +
                    "• Works on YouTube, FB, WA, TikTok
" +
                    "• No login needed
" +
                    "• Battery friendly
...
" +
                    "Just install → tap connect button."
                );
                b.setPositiveButton("Install WARP (Free)", (d, w) -> openWarp());
            ...
            if (dialog.getWindow() != null) {
                dialog.getWindow().setBackgroundDrawable(
                    new android.graphics.drawable.ColorDrawable(Color.parseColor("#1F1F1F")));
            }
            dialog.show();
    ...
    private void showMiniPlayer(String t, String s, String thumbUrl, boolean playing) {
        miniPlayerPlaying  = playing;
        miniPlayerThumbUrl = thumbUrl;

        if (miniPlayerLayout == null) buildMiniPlayer();

        // Update text + icon
        miniPlayerTitle.setText(t != null ? t : "");
        miniPlayerSub.setText(s != null ? s : "");
        miniPlayerPlayPause.setImageResource(playing
        ...
        if (thumbUrl != null && !thumbUrl.isEmpty()) {
            new Thread(() -> {
                try {
                    ...
                    runOnUiThread(() -> { if (bmp != null && miniPlayerThumb != null) miniPlayerThumb.setImageBitmap(bmp); });
                } catch (Exception ignored) {}
            }).start();
        }
        ...
        miniPlayerLayout.setTranslationY(dpToPx(80));
    ...
    private void hideMiniPlayer() {
        if (miniPlayerLayout == null) return;
        miniPlayerLayout.animate().alpha(0f).translationY(80f).setDuration(250)
            .withEndAction(() -> {
    ...
    private void buildMiniPlayer() {
        ...
        // ── Root card ────────────────────────────────────────────────────
        miniPlayerLayout = new RelativeLayout(this);

        android.graphics.drawable.GradientDrawable bg = new android.graphics.drawable.GradientDrawable();
        bg.setColor(Color.parseColor("#E61F1F1F"));
        bg.setCornerRadius(dpToPx(14));
        miniPlayerLayout.setBackground(bg);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            miniPlayerLayout.setElevation(dpToPx(10));
        }

        // ── Thumbnail ────────────────────────────────────────────────────
        miniPlayerThumb = new ImageView(this);
        RelativeLayout.LayoutParams tp = new RelativeLayout.LayoutParams(dpToPx(112), H);
        tp.addRule(RelativeLayout.ALIGN_PARENT_START);
        miniPlayerThumb.setLayoutParams(tp);
        miniPlayerThumb.setScaleType(ImageView.ScaleType.CENTER_CROP);
        miniPlayerThumb.setBackgroundColor(Color.parseColor("#2A2A2A"));
        miniPlayerThumb.setClipToOutline(true);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            android.graphics.drawable.GradientDrawable thumbShape = new android.graphics.drawable.GradientDrawable();
            thumbShape.setColor(Color.parseColor("#2A2A2A"));
            float[] radii = {dpToPx(14),dpToPx(14), 0,0, 0,0, dpToPx(14),dpToPx(14)};
            thumbShape.setCornerRadii(radii);
            miniPlayerThumb.setBackground(thumbShape);
        }
        miniPlayerLayout.addView(miniPlayerThumb);
        ...
        tlp.setMarginStart(dpToPx(120));
        tlp.setMarginEnd(dpToPx(84));
        ...
        miniPlayerTitle.setTextSize(12f);
        miniPlayerTitle.setTypeface(null, Typeface.BOLD);
        miniPlayerTitle.setMaxLines(1);
        miniPlayerTitle.setEllipsize(android.text.TextUtils.TruncateAt.END);

        miniPlayerSub = new TextView(this);
        miniPlayerSub.setTextColor(Color.parseColor("#AAAAAA"));
        miniPlayerSub.setTextSize(10f);
        miniPlayerSub.setMaxLines(1);
        miniPlayerSub.setEllipsize(android.text.TextUtils.TruncateAt.END);

        textLayout.addView(miniPlayerTitle);
        textLayout.addView(miniPlayerSub);
        miniPlayerLayout.addView(textLayout);

        // ── Play/Pause ───────────────────────────────────────────────────
        miniPlayerPlayPause = new ImageView(this);
        RelativeLayout.LayoutParams pp = new RelativeLayout.LayoutParams(dpToPx(38), dpToPx(38));
        pp.addRule(RelativeLayout.CENTER_VERTICAL);
        pp.addRule(RelativeLayout.ALIGN_PARENT_END);
        pp.setMarginEnd(dpToPx(46));
        miniPlayerPlayPause.setLayoutParams(pp);
        miniPlayerPlayPause.setImageResource(android.R.drawable.ic_media_pause);
        miniPlayerPlayPause.setColorFilter(Color.WHITE);
        miniPlayerPlayPause.setPadding(dpToPx(4), dpToPx(4), dpToPx(4), dpToPx(4));
        ...
        miniPlayerLayout.addView(miniPlayerPlayPause);

        // ── Close ────────────────────────────────────────────────────────
        miniPlayerClose = new ImageView(this);
        RelativeLayout.LayoutParams cp = new RelativeLayout.LayoutParams(dpToPx(34), dpToPx(34));
        cp.addRule(RelativeLayout.CENTER_VERTICAL);
        cp.addRule(RelativeLayout.ALIGN_PARENT_END);
        cp.setMarginEnd(dpToPx(6));
        miniPlayerClose.setLayoutParams(cp);
        miniPlayerClose.setImageResource(android.R.drawable.ic_menu_close_clear_cancel);
        miniPlayerClose.setColorFilter(Color.parseColor("#AAAAAA"));
        miniPlayerClose.setPadding(dpToPx(4), dpToPx(4), dpToPx(4), dpToPx(4));
        miniPlayerClose.setOnClickListener(v -> {
            hideMiniPlayer();
            web.evaluateJavascript("pauseVideo();", null);
            isPlaying = false;
        });
        miniPlayerLayout.addView(miniPlayerClose);

        // ── Tap thumbnail/text → expand ──────────────────────────────────
        miniPlayerThumb.setOnClickListener(v -> expandMiniPlayer());
        textLayout.setOnClickListener(v -> expandMiniPlayer());

        // ── Drag support ─────────────────────────────────────────────────
        miniPlayerLayout.setOnTouchListener((v, event) -> {
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    miniDragStartX = event.getRawX();
                    miniDragStartY = event.getRawY();
                    miniInitialX   = (int) miniPlayerLayout.getX();
                    miniInitialY   = (int) miniPlayerLayout.getY();
                    return true;
                case MotionEvent.ACTION_MOVE:
                    float dx = event.getRawX() - miniDragStartX;
                    float dy = event.getRawY() - miniDragStartY;
                    miniPlayerLayout.setX(miniInitialX + dx);
                    miniPlayerLayout.setY(miniInitialY + dy);
                    return true;
                case MotionEvent.ACTION_UP:
                    // Only snap if actually dragged, otherwise let click handlers fire
                    if (Math.abs(event.getRawX() - miniDragStartX) > dpToPx(8) ||
                        Math.abs(event.getRawY() - miniDragStartY) > dpToPx(8)) {
                        snapMiniPlayer();
                    }
                    return true;
            }
        ...
        // ── Add via WindowManager for proper overlay positioning ─────────
        ...
        // Initial position: bottom-right above nav bar
        wlp.gravity = Gravity.BOTTOM | Gravity.END;
        wlp.x = dpToPx(8);
        wlp.y = dpToPx(80);

        getWindowManager().addView(miniPlayerLayout, wlp);
        miniPlayerLayout.setVisibility(View.GONE);
    }
    private void expandMiniPlayer() {
        ...
        if (cur != null && cur.contains("youtube.com/watch")) {
    ...
    /** Snap mini player to left or right edge after drag */
    private void snapMiniPlayer() {
        if (miniPlayerLayout == null) return;
        float curX   = miniPlayerLayout.getX();
        int   screenW = getResources().getDisplayMetrics().widthPixels;
        int   playerW = dpToPx(320);
        float targetX = (curX + playerW / 2f > screenW / 2f)
            ? screenW - playerW - dpToPx(8)
            : dpToPx(8);
        miniPlayerLayout.animate()
            .x(targetX)
            .setDuration(220)
    ...
    /** onDestroy — remove WindowManager view to avoid leaking */
    private void destroyMiniPlayer() {
        if (miniPlayerLayout != null) {
            try { getWindowManager().removeView(miniPlayerLayout); } catch (Exception ignored) {}
            miniPlayerLayout = null;
        }
    }
    ...
    @Override public void onBackPressed() {
        ...
        // 2. If watching a video → show mini player instead of going back
        String currentUrl = web.getUrl();
        if (currentUrl != null && currentUrl.contains("youtube.com/watch") && isPlaying) {
            web.evaluateJavascript(
                "(function(){" +
                "  var v = document.querySelector('video');" +
                "  var t = document.querySelector('.slim-video-information-title, .ytm-media-lockup-title, h2.title');" +
                "  var s = document.querySelector('.slim-video-information-channel-name, .ytm-media-lockup-byline');" +
                "  var thumb = '';" +
                "  var img = document.querySelector('ytm-slim-media-lockup-metadata-renderer img, .ytm-media-lockup-image img, video');" +
                "  if(img && img.src && !img.src.includes('video')) thumb = img.src;" +
                "  if(!thumb){" +
                ...
                result -> {
                    if (result != null && !result.equals("null")) {
                        try {
                            ...
                            String t     = obj.optString("title", title);
                            String s     = obj.optString("sub",   subtitle);
                            String thumb = obj.optString("thumb", icon);
                            boolean paused = obj.optBoolean("paused", false);
                            runOnUiThread(() -> showMiniPlayer(t, s, thumb, !paused));
                        } catch (Exception e) {
                            runOnUiThread(() -> showMiniPlayer(title, subtitle, icon, isPlaying));
                        }
                    } else {
                        runOnUiThread(() -> showMiniPlayer(title, subtitle, icon, isPlaying));
                    }
