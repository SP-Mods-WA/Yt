...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Pre-build scripts in background
        ...
        if (!isNetworkAvailable()) {
            ...
            // System bars AFTER WebView starts loading
            mainHandler.postDelayed(this::setupSystemBarsInsets, 50);
            
            // ALL heavy tasks deferred by 3 seconds
    ...
    private void showLoadingScreen() {
        // Don't show loading screen - makes app feel slower
        // Real YouTube app doesn't show loading on startup
    }
    ...
    private void configureWebSettings() {
        ...
        // ── CRITICAL: AGGRESSIVE cache for INSTANT loading ───────────────
        s.setCacheMode(WebSettings.LOAD_CACHE_ELSE_NETWORK);
        // This mode is actually FASTER for YouTube:
        // - Serves cached content INSTANTLY (0ms)
        // - Updates cache in background if needed
        // Real YouTube app behavior
        
        s.setAppCacheEnabled(true);
        s.setAppCachePath(getCacheDir().getAbsolutePath());
        s.setAppCacheMaxSize(100 * 1024 * 1024); // 100MB cache
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  Optimized WebViewClient - REAL YOUTUBE SPEED
    // ══════════════════════════════════════════════════════════════════════════
    private class OptimizedWebViewClient extends WebViewClient {
        ...
        public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {
            ...
            if (newUrl.contains("/shorts") && !userNavigated) {
                ...
                if (cur != null && !cur.contains("/shorts")) return true;
            }
            userNavigated = false;
            return false;
        }
        ...
        public void onPageFinished(WebView v, String url) {
            ...
            // INSTANT script injection - no delays
            if (!scriptsInjected) {
                injectYTProScriptsFromAssets();
                scriptsInjected = true;
            }

            // Navigation hiding - INSTANT
            ...
            // INSTANT play trigger for videos
            if (url.contains("youtube.com/watch")) {
                triggerInstantPlay();
                
                // Second trigger for safety
                mainHandler.postDelayed(() -> triggerInstantPlay(), 300);
            }

            // Background optimization ONCE only
    ...
    private static final String JS_NOTIFICATION_PAGE =
        ...
        "st.innerHTML='*{margin:0;padding:0}html,body{margin:0!important;padding:0!important;width:100%!important}ytm-mobile-topbar-renderer,ytm-pivot-bar-renderer,#masthead{display:none!important}body{padding-top:0!important;padding-bottom:70px!important;background:#0F0F0F!important}';" +
        "document.head.appendChild(st);" +
        "})();";

    private static final String JS_HIDE_NATIVE_NAV =
        "(function(){" +
        "if(window.__ytproNavHidden)return;window.__ytproNavHidden=1;" +
        "var css='ytm-mobile-topbar-renderer,#masthead,.mobile-topbar-header,ytm-pivot-bar-renderer,ytm-pivot-bar-item-renderer,.pivot-bar-item-tab,.pivot-bar,c3-tab-bar-renderer{display:none!important;height:0!important;opacity:0!important}body{padding-top:0!important;padding-bottom:70px!important}#page-manager{padding-bottom:70px!important}';" +
        "var st=document.createElement('style');st.textContent=css;document.head.appendChild(st);" +
        "function h(){['ytm-mobile-topbar-renderer','#masthead','ytm-pivot-bar-renderer','ytm-pivot-bar-item-renderer','c3-tab-bar-renderer'].forEach(function(s){document.querySelectorAll(s).forEach(function(e){e.style.display='none';});});}" +
        "h();" +
        "new MutationObserver(h).observe(document.body,{childList:1,subtree:1});" +
        "})();";

    private static final String JS_BLOCK_SHORTS_PUSH =
        "(function(){" +
        "if(window.__ytproShortBlocked)return;window.__ytproShortBlocked=1;" +
        "var o=history.pushState;" +
        "history.pushState=function(s,t,u){if(u&&u.includes('/shorts')&&!location.href.includes('/shorts'))return;return o.apply(this,arguments);};" +
        "})();";

    // ══════════════════════════════════════════════════════════════════════════
    //  Script injection - INSTANT
    // ══════════════════════════════════════════════════════════════════════════
    ...
    private void injectYTProScriptsFromAssets() {
        // 1. VIDEO PLAYER BOOST FIRST (critical for speed)
        ...
        // 3. Other scripts (async, non-blocking)
        String loader = (cachedScriptLoader != null) ? cachedScriptLoader : buildScriptLoader();
        web.evaluateJavascript(loader, null);

        // 4. Status bar (last, lowest priority)
        mainHandler.postDelayed(() -> 
            web.evaluateJavascript(JS_STATUS_BAR_SYNC, null), 300);
    }
    ...
    private String buildScriptLoader() {
        ...
        sb.append("function L(c){try{var s=document.createElement('script');s.textContent=c;document.body.appendChild(s);}catch(e){}}");
        ...
        sb.append("window.YTPRO_LOADED=1;})();");
        return sb.toString();
    }
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  ULTRA FAST VIDEO PLAYER - Real YouTube behavior! 🚀
    // ══════════════════════════════════════════════════════════════════════════
    private static final String JS_PLAYER_BOOST =
        "(function(){" +
        "if(window.__ytproBoost)return;window.__ytproBoost=1;" +
        ...
        "if(v.paused){setTimeout(function(){v.play().catch(function(){});},50);}" +
        "});" +
        "}" +
        
        // ── YouTube player config ─────────────────────────────────────────
        "if(window.ytplayer&&window.ytplayer.config){" +
        "var c=window.ytplayer.config;" +
        "c.args=c.args||{};" +
        "c.args.autoplay='1';" +
        ...
        // ── REAL YOUTUBE BUFFER SETTINGS ──────────────────────────────────
        "if(window.yt&&window.yt.config_){" +
        "var f=window.yt.config_.EXPERIMENT_FLAGS=window.yt.config_.EXPERIMENT_FLAGS||{};" +
        
        // CRITICAL: Real YouTube mobile values
        "f.html5_min_readahead_seconds=0.1;" +               // Start at 0.1s (INSTANT)
        "f.html5_max_readahead_seconds=180;" +               // Buffer up to 3 min
        "f.html5_max_buffer_seconds=180;" +
        "f.html5_platform_minimum_readahead_seconds=0.1;" +
        
        // NO limits
        "f.html5_ignore_bandwidth_cap=true;" +
        "f.html5_disable_stop_buffering=false;" +
        "f.html5_prefer_server_bandwidth_cap=false;" +
        "f.html5_max_readahead_bandwidth_cap=-1;" +
        
        // Fast response
        "f.html5_subsegment_readahead_seek_latency_fudge=0.01;" +
        "f.html5_deadzone_multiplier=0;" +
        
        // Quality
        "f.html5_enable_vp9_on_mobile=true;" +
        "f.html5_enable_partial_initial_response=true;" +
        "f.kevlar_miniplayer_expand_top=true;" +
        "f.html5_bandwidth_window_size=180;" +
        "}" +
        
        // ── Report FAST connection to YouTube ─────────────────────────────
        "try{" +
        "if(!navigator.connection)navigator.connection={};" +
        "Object.defineProperty(navigator.connection,'effectiveType',{get:function(){return '4g'},configurable:1});" +
        "Object.defineProperty(navigator.connection,'downlink',{get:function(){return 200},configurable:1});" +
        "Object.defineProperty(navigator.connection,'rtt',{get:function(){return 5},configurable:1});" +
        "Object.defineProperty(navigator.connection,'saveData',{get:function(){return false},configurable:1});" +
        "}catch(e){}" +
        
        // ── Remove ads FAST ───────────────────────────────────────────────
        "setTimeout(function(){" +
        "document.querySelectorAll('ytm-purchase-offer-renderer,ytm-upsell-dialog-renderer,.ytp-ad-overlay-container,.ytp-ad-text-overlay').forEach(function(e){e.remove();});" +
        "prep();" +
        "},100);" +
        
        // ── Watch for new videos ──────────────────────────────────────────
        "prep();" +
        "var obs=new MutationObserver(prep);" +
        "obs.observe(document.body,{childList:1,subtree:1});" +
        
        "})();";

    // ══════════════════════════════════════════════════════════════════════════
    //  INSTANT PLAY - Real YouTube behavior
    // ══════════════════════════════════════════════════════════════════════════
    private void triggerInstantPlay() {
        if (web == null) return;
        web.evaluateJavascript(
            "(function(){" +
    ...
    private static final String JS_STATUS_BAR_SYNC =
        ...
        "function u(){var h=document.querySelector('ytm-mobile-topbar-renderer')||document.querySelector('#masthead');" +
        "if(h&&window.Android&&window.Android.setStatusBarColor){" +
        "var c=rgb2hex(getComputedStyle(h).backgroundColor);" +
        "window.Android.setStatusBarColor(c);}}" +
        "new MutationObserver(u).observe(document.body,{attributes:1,childList:1,subtree:1});" +
        "setTimeout(u,200);" +
        "})();";
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            networkCallback = new ConnectivityManager.NetworkCallback() {
                @Override public void onAvailable(Network network) {
                    mainHandler.postDelayed(() -> onNetworkRestored(), 300);
                }
    ...
    private void setupCustomHeader() {
        ...
        iconSearch.setOnClickListener(v -> {
            ...
            searchBarContainer.animate().alpha(1f).setDuration(150).start();
    ...
    private void closeSearchBar(RelativeLayout container, EditText input) {
        container.animate().alpha(0f).setDuration(150)
            .withEndAction(() -> {
    ...
    private void startVoiceSearch(EditText input, ArrayAdapter<String> adapter, ListView list) {
        ...
        sr.setRecognitionListener(new android.speech.RecognitionListener() {
            @Override public void onResults(Bundle b) {
                ...
                if (m != null && !m.isEmpty()) {
                    String q = m.get(0);
                    runOnUiThread(() -> { input.setText(q); input.setSelection(q.length()); });
                }
                sr.destroy();
            }
            @Override public void onPartialResults(Bundle b) {
                ...
                if (p != null && !p.isEmpty()) {
                    runOnUiThread(() -> input.setText(p.get(0)));
                }
            }
            @Override public void onError(int e) { sr.destroy(); }
    ...
    private void showMiniPlayer(String t, String s, String thumbUrl, boolean playing) {
        ...
        miniPlayerLayout.animate().alpha(1f).setDuration(200).start();
        isMiniPlayerVisible = true;
    }

    private void hideMiniPlayer() {
        if (miniPlayerLayout == null) return;
        miniPlayerLayout.animate().alpha(0f).setDuration(150)
            .withEndAction(() -> {
        ...
        miniPlayerLayout.setOnTouchListener((v, event) -> {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    miniPlayerLayout.setX(miniInitialX + event.getRawX() - miniDragStartX);
                    miniPlayerLayout.setY(miniInitialY + event.getRawY() - miniDragStartY);
                    return true;
                case MotionEvent.ACTION_UP:
                    if (Math.abs(event.getRawX() - miniDragStartX) > dpToPx(8)) snapMiniPlayer();
                    return true;
            }
            return false;
        });
    ...
    private void snapMiniPlayer() {
        ...
        float targetX = (curX + playerW / 2f > screenW / 2f) ? screenW - playerW - dpToPx(8) : dpToPx(8);
        miniPlayerLayout.animate().x(targetX).setDuration(150).start();
    }
    ...
    @Override public void onBackPressed() {
        ...
        if (currentUrl != null && currentUrl.contains("youtube.com/watch") && isPlaying) {
            web.evaluateJavascript(
                "(function(){" +
                "var v=document.querySelector('video');" +
                "var t=document.querySelector('.slim-video-information-title, .ytm-media-lockup-title');" +
                "var s=document.querySelector('.slim-video-information-channel-name, .ytm-media-lockup-byline');" +
                "var thumb='';" +
                "var img=document.querySelector('img[src*=\"ytimg.com\"]');" +
                ...
                result -> {
                    if (result != null && !result.equals("null")) {
                        try {
                            String json = result.replace("\\\"", "\"").replace("\\\\", "\\");
                            if (json.startsWith("\"")) json = json.substring(1, json.length() - 1);
                            JSONObject obj = new JSONObject(json);
                            runOnUiThread(() -> showMiniPlayer(
                                obj.optString("title", title),
                                obj.optString("sub", subtitle),
                                obj.optString("thumb", icon),
                                !obj.optBoolean("paused", false)
                            ));
                        } catch (Exception ignored) {
    ...
    @Override public void onRequestPermissionsResult(int code, String[] perms, int[] results) {
        ...
        if (code == 101 && results.length > 0 && results[0] == PackageManager.PERMISSION_GRANTED) {
            web.loadUrl("https://m.youtube.com");
        } else if (code == 102 && results.length > 0 && results[0] == PackageManager.PERMISSION_GRANTED) {
            startNotificationService();
        }
    }
    ...
    private void checkNotificationsNow() {
        new NotificationFetcher(this).fetchNotifications(new NotificationFetcher.NotificationCallback() {
            @Override public void onSuccess(List<NotificationModel> n) { 
                new AppNotificationManager(MainActivity.this).showNotifications(n); 
            }
    ...
    private void fetchAndUpdateNotifications() {
        notificationFetcher.fetchNotifications(new NotificationFetcher.NotificationCallback() {
            @Override public void onSuccess(List<NotificationModel> n) {
                runOnUiThread(() -> { 
                    notificationPrefs.saveNotifications(n); 
    ...
    private void showOfflineScreen() {
        ...
        msg.setText("Check your connection");
        msg.setTextSize(14); msg.setTextColor(Color.parseColor("#AAAAAA"));
        center.addView(msg);
    ...
    private void setupSystemBarsInsets() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            root.setOnApplyWindowInsetsListener((v, insets) -> {
                ...
                applyInsetsToViews(bars.top, bars.bottom);
            ...
            root.post(() -> applyInsetsToViews(getStatusBarHeight(), getNavigationBarHeight()));
        }
        root.requestApplyInsets();
    }

    private void applyInsetsToViews(int top, int bottom) {
        runOnUiThread(() -> {
            View header = findViewById(R.id.customHeader);
            if (header != null) {
                RelativeLayout.LayoutParams p = (RelativeLayout.LayoutParams) header.getLayoutParams();
                p.topMargin = top; header.setLayoutParams(p);
            }
            View nav = findViewById(R.id.bottomNavBar);
            if (nav != null) {
                RelativeLayout.LayoutParams p = (RelativeLayout.LayoutParams) nav.getLayoutParams();
                p.bottomMargin = bottom; nav.setLayoutParams(p);
            }
        });
    }
