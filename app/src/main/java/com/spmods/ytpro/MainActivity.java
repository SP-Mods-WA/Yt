...
// ✅ Settings - Open YTPRO settings
  ...
  public class WebAppInterface {
    ...
    @JavascriptInterface
public void playNativeVideo(String videoUrl, String videoTitle, long currentTime) {
    runOnUiThread(() -> {
        Intent intent = new Intent(MainActivity.this, NativePlayerActivity.class);
        intent.putExtra("video_url", videoUrl);
        intent.putExtra("video_title", videoTitle);
        intent.putExtra("resume_position", currentTime);
...
protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (requestCode == 100 && resultCode == RESULT_OK && data != null) {
        long resumePos = data.getLongExtra("resume_position", 0);
        web.evaluateJavascript("seekTo('" + (resumePos / 1000) + "');", null);
    }
}
}
