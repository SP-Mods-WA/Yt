...
public class MainActivity extends Activity {
  ...
  private void setupCustomHeader() {
    ...
    iconSearch.setOnClickListener(v -> {
        ...
        web.evaluateJavascript(
            "(function() {" +
            "  var searchBox = document.querySelector('ytm-search input, ytm-search-box input, input[type=\"search\"]');" +
            "  if (searchBox) {" +
            "    searchBox.focus();" +
            "    searchBox.click();" +
            "  } else {" +
            "    window.location.href = 'https://m.youtube.com/results?search_query=';" +
            "  }" +
            "})();",
            null
        );
