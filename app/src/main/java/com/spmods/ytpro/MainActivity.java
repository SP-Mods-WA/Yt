...
public void onPageFinished(WebView p1, String url) {
    ...
    // ✅✅ FORCE HIDE YOUTUBE UI - Enhanced Version ✅✅
    web.evaluateJavascript(
        "(function() {" +
        "  function forceHideYouTubeUI() {" +
        // Create persistent style element
        "    if (!document.getElementById('ytpro-force-hide')) {" +
        "      var style = document.createElement('style');" +
        "      style.id = 'ytpro-force-hide';" +
        "      style.innerHTML = '" +
        // Hide ALL possible YouTube navigation elements
        ...
        "      document.head.appendChild(style);" +
        "    }" +
        // Forcefully remove elements from DOM
        "    var selectors = [" +
        "      'ytm-mobile-topbar-renderer'," +
        "      'ytm-pivot-bar-renderer'," +
        "      'ytm-pivot-bar-item-renderer'," +
        ...
        // Run immediately
        "  forceHideYouTubeUI();" +
        // Watch for DOM changes (YouTube's SPA navigation)
        ...
        "  });" +
        // Aggressive fallback interval
        "  setInterval(forceHideYouTubeUI, 300);" +
        // Also run after page transitions
        "  if (window.navigation) {" +
        "    window.navigation.addEventListener('navigate', forceHideYouTubeUI);" +
        "  }" +
        "  console.log('✅ YTPRO: Force hiding YouTube UI active');" +
        "})();",
        null
    );
