    ...
    web.setWebViewClient(new WebViewClient() {
      ...
      public void onPageFinished(WebView p1, String url) {
        ...
        p1.evaluateJavascript(
            ...
            "    }" +
            "    " +
            "    /* ✅ Video controls - YouTube app style */" +
            "    .player-controls-top, " +
            ...
            "    .yt-progress-bar-line-progress-bar-played { " +
            "      background: #FF0000 !important; " +
            "    }" +
            "    " +
            "    .ytProgressBarLineProgressBarPlayhead, " +
            "    .yt-progress-bar-line-progress-bar-playhead { " +
            "      background: #FF0000 !important; " +
            "      box-shadow: 0 0 4px rgba(255, 0, 0, 0.8) !important; " +
            "    }" +
            "    " +
            "    /* ✅ Buffered segments */" +
            "    .ytProgressBarLineProgressBarBuffered { " +
        ...
        web.evaluateJavascript(
            ...
            "    var video = document.querySelector('.video-stream');" +
            "    if (video && video.src) {" +
            "      var qualityMatch = video.src.match(/itag=(\\d+)/);" +
            "      var currentQuality = qualityMatch ? qualityMatch[1] : null;" +
            "      if (currentQuality && currentQuality !== lastQuality) {" +
            "        lastQuality = currentQuality;" +
            "        console.log('Quality changed to itag:', currentQuality);" +
            "      }" +
            "    }" +
            "  }, 2000);" +
            "  " +
  ...
  private void injectYTProScriptsFromAssets() {
    try {
        ...
        String scriptLoader = 
            ...
            "  " + loadScriptFromAssets("video-enhancements.js") + " " + // ✅ මෙන්න මේකයි add කරන්න ඕනේ!
