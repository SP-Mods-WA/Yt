...
// ✅ Settings - Open YTPRO settings
  ...
  public void onPictureInPictureModeChanged(boolean isInPictureInPictureMode, Configuration newConfig) {
    ...
    // ✅ Save PIP state to SharedPreferences for WebView access
    SharedPreferences prefs = getSharedPreferences("YTPRO", MODE_PRIVATE);
    prefs.edit().putBoolean("isPIP", isInPictureInPictureMode).apply();
    Log.d("PIP", "💾 PIP state saved: " + isInPictureInPictureMode);
  ...
  protected void onPause() {
    ...
    // ✅ DON'T pause WebView if in PIP mode
    if (!isPip) {
        // Only pause WebView when NOT in PIP mode
        if (web != null) {
            web.onPause();
            Log.d("MainActivity", "📱 WebView paused (not in PIP)");
        }
    } else {
        Log.d("MainActivity", "🔒 WebView NOT paused (PIP mode active)");
    }
  ...
  protected void onResume() {
    ...
    if (web != null) {
        web.onResume();
        Log.d("MainActivity", "▶️ WebView resumed");
    }
  }
