...
public class MainActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    // ✅ Set status bar to match YouTube
  ...
  public void load(boolean dl) {
    ...
    // ✅ FIXED: WebViewClient with all fixes in correct place
    web.setWebViewClient(new WebViewClient() {
      
      @Override
      public void onPageFinished(WebView p1, String url) {
        
        if (!userNavigated && !url.equals(lastUrl)) {
            lastUrl = url;
            userNavigated = true;
        }
        
        // ✅ CSS injection - runs for ALL pages
        web.evaluateJavascript(
            "(function() {" +
            "  var style = document.createElement('style');" +
            "  style.id = 'ytpro-enhanced-styles';" +
            "  style.textContent = '" +
            "    .html5-video-container { position: absolute !important; width: 100% !important; height: 100% !important; top: 0 !important; left: 0 !important; }" +
            "    .video-stream.html5-main-video { position: absolute !important; top: 0 !important; left: 0 !important; width: 100% !important; height: 100% !important; object-fit: contain !important; }" +
            "    .ytp-time-display, .ytp-time-current, .ytp-time-duration { opacity: 1 !important; visibility: visible !important; display: inline-block !important; }" +
            "    .ytp-button, .ytp-settings-button, .ytp-subtitles-button { opacity: 1 !important; visibility: visible !important; }" +
            "    ytm-mobile-topbar-renderer { display: none !important; }" +
            "    body { padding-top: 56px !important; padding-bottom: 65px !important; }" +
            "  ';" +
            "  if (!document.getElementById('ytpro-enhanced-styles')) { document.head.appendChild(style); }" +
            "})();",
            null
        );
        ...
        web.evaluateJavascript(
            ...
            "    if (v) { v.style.position='absolute'; v.style.left='0'; v.style.top='0'; }" +
            "    document.querySelectorAll('.ytp-button, .ytp-time-display').forEach(function(c) { c.style.opacity='1'; });" +
            "  }" +
            "  setInterval(fixVideo, 1500);" +
            "  fixVideo();" +
            "})();",
            null
        );
        
        // Shorts redirect block
        web.evaluateJavascript(
            "(function() {" +
  ...
  private void injectYTProScriptsFromAssets() {
    try {
        ...
        String scriptLoader = 
            ...
            "  " + loadScriptFromAssets("videoenhancements.js") + " " +
