...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!isNetworkAvailable()) {
            ...
            // MINIMAL startup - just load
            showLoadingScreen();
            load(false);
            setupNetworkMonitor();
            
            // DEFER heavy tasks until after page loads
    ...
    private void hideLoadingScreen() {
        runOnUiThread(() -> {
            ...
            loadingScreen.animate().alpha(0f).setDuration(150)
    ...
    private void configureWebSettings() {
        ...
        // ── CRITICAL: Best cache mode for speed ──────────────────────────
        s.setCacheMode(WebSettings.LOAD_DEFAULT);
        // Uses cache if fresh, dramatically faster on repeat visits
        
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            s.setDatabasePath(getDir("databases", Context.MODE_PRIVATE).getPath());
        }

        // ── HARDWARE ACCELERATION (critical for video) ────────────────────
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  Optimized WebViewClient - INSTANT LOADING
    // ══════════════════════════════════════════════════════════════════════════
    private class OptimizedWebViewClient extends WebViewClient {
        ...
        public void onPageFinished(WebView v, String url) {
            ...
            // INSTANT hide loading
            hideLoadingScreen();

            // AUTO-PLAY trigger for videos
            if (url.contains("youtube.com/watch")) {
                mainHandler.postDelayed(() -> triggerInstantPlay(), 200);
            }

            // Background tasks AFTER first load only
    ...
    private void injectYTProScriptsFromAssets() {
        // 1. PLAYER BOOST FIRST (most critical for video speed)
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  ULTRA FAST VIDEO PLAYER - මෙතන තමයි වැඩේ වෙන්නේ! 🚀
    // ══════════════════════════════════════════════════════════════════════════
    private static final String JS_PLAYER_BOOST =
        "(function(){" +
        "if(window.__ytproBoost)return;window.__ytproBoost=true;" +
        
        // ── INSTANT video element prep ───────────────────────────────────
        "function prepVideo(){" +
        ...
        // Force immediate load if not started
        "    if(v.readyState<2){" +
        "      v.load();" +
        "      setTimeout(function(){" +
        ...
        // ── CRITICAL: Aggressive buffer settings ─────────────────────────
        "if(window.yt&&window.yt.config_){" +
        "  var f=window.yt.config_.EXPERIMENT_FLAGS=window.yt.config_.EXPERIMENT_FLAGS||{};" +
        
        // INSTANT playback - no waiting
        "  f.html5_min_readahead_seconds=0.5;" +            // Start playing at 0.5s buffer
        "  f.html5_platform_minimum_readahead_seconds=0.5;" + // Platform minimum
        "  f.html5_subsegment_readahead_target_buffer_health_seconds=0.5;" +
        
        // MAXIMUM buffering for smooth playback
        "  f.html5_max_readahead_seconds=120;" +            // Buffer up to 2 minutes
        "  f.html5_max_buffer_seconds=120;" +
        
        // NO restrictions
        "  f.html5_ignore_bandwidth_cap=true;" +            // Ignore ISP caps
        "  f.html5_disable_stop_buffering=false;" +         // Keep buffering
        "  f.html5_prefer_server_bandwidth_cap=false;" +    // No server limits
        "  f.html5_max_readahead_bandwidth_cap=-1;" +       // Unlimited bandwidth
        
        // Fast seeking
        "  f.html5_subsegment_readahead_seek_latency_fudge=0.05;" +
        ...
        "  f.html5_bandwidth_window_size=120;" +
        "}" +
        ...
        "    get:function(){return 100;},configurable:true" +  // Report 100 Mbps
        "  });" +
        "  Object.defineProperty(navigator.connection,'rtt',{" +
        "    get:function(){return 10;},configurable:true" +   // Low latency
        "  });" +
        ...
        "  prepVideo();" +  // Re-prep in case new videos loaded
        "},200);" +
        
        // ── Watch for new videos (SPA navigation) ────────────────────────
        "var obs=new MutationObserver(function(){prepVideo();});" +
        "obs.observe(document.body,{childList:true,subtree:true});" +
        
        "})();";

    // ══════════════════════════════════════════════════════════════════════════
    //  INSTANT PLAY TRIGGER - Video එක instant play කරන්න
    // ══════════════════════════════════════════════════════════════════════════
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            networkCallback = new ConnectivityManager.NetworkCallback() {
                @Override public void onAvailable(Network network) {
                    mainHandler.postDelayed(() -> onNetworkRestored(), 500);
                }
    ...
    // ══════════════════════════════════════════════════════════════════════════
    //  Quick DoH pre-resolve - MINIMAL version
    // ══════════════════════════════════════════════════════════════════════════
    private void checkConnectionSpeed() {
        new Thread(() -> {
            try {
                ...
                String[] criticalDomains = {"m.youtube.com", "googlevideo.com"};
                
                for (String domain : criticalDomains) {
                    try {
                        URL dohUrl = new URL(
                        ...
                        doh.setConnectTimeout(1500);
                        doh.setReadTimeout(1500);
                        doh.connect();
                        doh.getInputStream().close();
                        doh.disconnect();
                    } catch (Exception ignored) {}
                }
            } catch (Exception ignored) {}
    ...
    private void setupCustomHeader() {
        ...
        iconSearch.setOnClickListener(v -> {
            ...
            searchBarContainer.animate().alpha(1f).translationY(0f).setDuration(200).start();
    ...
    private void closeSearchBar(RelativeLayout container, EditText input) {
        container.animate().alpha(0f).translationY(-56f).setDuration(200)
            .withEndAction(() -> {
    ...
    private void hideMiniPlayer() {
        ...
        miniPlayerLayout.animate().alpha(0f).translationY(80f).setDuration(200)
            .withEndAction(() -> {
