...
// Handle skip to previous
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null) {
            ...
            // ✅ Acquire wake lock when service starts with play action
            String action = intent.getStringExtra("action");
            if ("play".equals(action) && wakeLock != null && !wakeLock.isHeld()) {
                wakeLock.acquire(10*60*1000L); // 10 minutes
                Log.d("ForegroundService", "🔒 Wake lock acquired");
            } else if ("pause".equals(action) && wakeLock != null && wakeLock.isHeld()) {
                wakeLock.release();
                Log.d("ForegroundService", "🔓 Wake lock released on pause");
            }
        }
    ...
    public void onDestroy() {
        ...
        if (wakeLock != null && wakeLock.isHeld()) {
            wakeLock.release();
            Log.d("ForegroundService", "🔓 Wake lock released on destroy");
        }
