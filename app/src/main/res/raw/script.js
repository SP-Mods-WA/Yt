...
html+=`<button class="dl-btn" data-url="${f.url}" data-name="${title}.mp4" data-type="video/mp4">📹 ${q} ${s}</button>`;
});
html+=`</div>`;
}
...
const s=f.contentLength?formatFileSize(f.contentLength):"";
html+=`<button class="dl-btn" data-url="${f.url}" data-name="${title}_${q}.mp4" data-type="video/mp4">🎬 ${q}${fps} ${s}</button>`;
});
html+=`</div>`;
}
...
const s=f.contentLength?formatFileSize(f.contentLength):"";
html+=`<button class="dl-btn" data-url="${f.url}" data-name="${title}.m4a" data-type="audio/mp4">🎵 ${br} ${s}</button>`;
});
html+=`</div>`;
}
...
html+=`<button class="dl-btn" data-url="${t.url}" data-name="${title}_thumb.jpg" data-type="image/jpeg">📸 ${t.width} × ${t.height}</button>`;
html+=`</div>`;
}

ytproDownDiv.innerHTML=html;

// FIX: download buttons delegation
...
ytproDownDiv.innerHTML=`<div style="padding:20px;color:${c};"><h3>❌ Error</h3><p style="margin-top:10px;font-size:13px;">${error.message}</p><button data-action="close" style="margin-top:15px;padding:10px 20px;background:${c};color:${dc};border:0;border-radius:10px;">Close</button></div>`;
