...
YTPROCodecs.video.forEach((x)=>{
    localStorage.setItem(x,"true");  // true = enabled
});
...
function makeModifiedTypeChecker(origChecker) {
    return function (type) {
        ...
        // FIXED: Block only if set to FALSE (disabled)
        ...
        if (localStorage['AV1'] === 'false') {  // ✅ CHANGED from 'true' to 'false'
...
<br>
<button onclick="Android.oplink('mailto:spmodsofficial@gmail.com')">Report Bugs
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="${isD ? "#ccc" : "#444"}" viewBox="0 0 16 16">
<path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708"/>
</svg>
</button>
...
// Removed fetch calls since files are local
// fetch('file:///android_asset/script.js', {cache: 'reload'});
// fetch('file:///android_asset/bgplay.js', {cache: 'reload'});
// fetch('file:///android_asset/innertube.js', {cache: 'reload'});
}
...
async function ytproDownVid(){
    ...
    try {
        ...
        #downytprodiv .section {
            margin: 15px 0;
            text-align: left;
        }
        #downytprodiv .section-title {
            font-size: 13px;
            font-weight: 700;
            color: ${c};
            margin: 10px 0 8px 0;
            padding: 8px 10px;
            background: ${isD ? 'rgba(66,133,244,0.15)' : 'rgba(6,95,212,0.1)'};
            border-left: 3px solid ${isD ? '#4285f4' : '#065fd4'};
            border-radius: 5px;
        }
        #downytprodiv .download-btn {
            display: flex;
            align-items: center;
        ...
        #downytprodiv .download-btn:active {
            transform: scale(0.97);
        }
        #downytprodiv .btn-quality {
            font-weight: 600;
        }
        #downytprodiv .btn-size {
            font-size: 11px;
            opacity: 0.8;
        }
        </style>`;
...
// ✅ Raw resource එකෙන් load කරන්න - Android interface එක call කරන්න
if(typeof Android !== 'undefined' && Android.getScript){
    var script2 = document.createElement('script');
    script2.textContent = Android.getScript();
...
var script = doc.createElement("script");
// ✅✅✅ Raw resource එකෙන් script load කරන්න ✅✅✅
var scriptSource=`window.addEventListener('DOMContentLoaded', function() {
...
function makeDraggable(element) {
    ...
    element.addEventListener('touchstart', function(e) {
        // Close button එක click කරොත් drag නොකරන්න
...
// Video සහ icon size adjust කරන function
function adjustVideoAndIcons(miniMode) {
    if (miniMode) {
        // Mini mode - සියලුම child elements හොයාගන්න
        var video = player.querySelector('video');
        var videoContainer = player.querySelector('.html5-video-container');
        var mainContainer = player.querySelector('.html5-main-video');
        
        // Video element එක adjust කරන්න
        if (video) {
        ...
        // Main video container adjust කරන්න
        if (mainContainer) {
            mainContainer.style.width = '100%';
            mainContainer.style.height = '100%';
        }
        
        // Player එකේ සියලුම direct children එවා proper display කරන්න
        Array.from(player.children).forEach(function(child) {
            if (!child.classList.contains('mini-close-btn')) {
                var computed = getComputedStyle(child);
                if (computed.position === 'absolute' || computed.position === 'relative') {
                    // Position එක හරියට තියාගන්න
                }
            }
        });
        
        // Icons/controls සඳහා size adjust කරන්න
        var controls = player.querySelectorAll('button:not(.mini-close-btn), .ytp-button');
        controls.forEach(function(control) {
            control.style.transform = 'scale(0.8)';
        });
        
    } else {
        // Normal mode - reset කරන්න
        var video = player.querySelector('video');
        var videoContainer = player.querySelector('.html5-video-container');
        var mainContainer = player.querySelector('.html5-main-video');
...
// Close button create කරන function
function createCloseButton() {
    // දැනටමත් close button එකක් තියෙනවද බලන්න
    ...
    // Hover effect (touch devices සඳහා active state)
    ...
    // ✅✅✅ FIXED: Close button click කරද්දි minimize mode එකෙන් exit වෙන්න ✅✅✅
    closeBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        
        // Minimize mode එකෙන් exit වෙන්න
        minimize(false);
    });
...
// Close button remove කරන function
...
// Mini player size (16:9 aspect ratio)
var miniWidth = window.innerWidth * 0.45; // 45% of screen width
var miniHeight = miniWidth * (9/16); // 16:9 aspect ratio

// Smooth transitions and rounded corners
player.style.display="block";
player.style.transition="all 0.3s ease";
player.style.transform="scale(1)"; // Reset scale
...
// Video සහ icons adjust කරන්න mini mode සඳහා
setTimeout(function() {
    adjustVideoAndIcons(true);
}, 50);

// Close button එක add කරන්න
createCloseButton();

// Make draggable
if(!player.hasAttribute("data-draggable")){
    makeDraggable(player);
    player.setAttribute("data-draggable","true");
}
...
// Video සහ icons reset කරන්න
adjustVideoAndIcons(false);

// Close button එක remove කරන්න
removeCloseButton();

player.removeAttribute("ogTop");
player.removeAttribute("ogLeft");
player.removeAttribute("ogPosition");
player.removeAttribute("ogWidth");
player.removeAttribute("ogHeight");
player.removeAttribute("ogTransform");
player.removeAttribute("ogBorderRadius");
player.removeAttribute("ogZIndex");
player.removeAttribute("data-draggable");
...
// ✅ Immediately prevent any pause attempts
setTimeout(() => {
    if (v.paused) {
        v.play().catch(e => console.log('PIP play failed:', e));
    }
}, 50);

// ✅ Keep forcing play every 500ms in PIP mode
window.pipPlayInterval = setInterval(() => {
    if (window.isPIP && v.paused) {
        console.log('🔄 Auto-resuming video in PIP mode');
        v.play().catch(e => {});
    }
}, 500);

console.log('✅ PIP mode active - auto-resume enabled');
...
// ✅ Override Page Visibility API for PIP mode
// This prevents YouTube from pausing when screen turns off in PIP mode
(function() {
    let hiddenProp = 'hidden';
    let visibilityChange = 'visibilitychange';
    
    // Find the correct vendor prefix
    ...
    let originalHiddenDescriptor = Object.getOwnPropertyDescriptor(Document.prototype, hiddenProp) || 
                                   Object.getOwnPropertyDescriptor(document, hiddenProp);
    
    // Override document.hidden property
    Object.defineProperty(document, hiddenProp, {
        get: function() {
            // If in PIP mode, always return false (visible)
            if (window.isPIP) {
                console.log('🔒 Faking document.hidden = false (PIP mode)');
                return false;
            }
            // Otherwise use original behavior
            return originalHiddenDescriptor ? originalHiddenDescriptor.get.call(this) : false;
        },
        configurable: true
    });
    
    // Override document.visibilityState
    let originalVisibilityDescriptor = Object.getOwnPropertyDescriptor(Document.prototype, 'visibilityState') ||
                                       Object.getOwnPropertyDescriptor(document, 'visibilityState');
    
    Object.defineProperty(document, 'visibilityState', {
        get: function() {
            // If in PIP mode, always return 'visible'
            if (window.isPIP) {
                console.log('🔒 Faking document.visibilityState = visible (PIP mode)');
                return 'visible';
            }
            // Otherwise use original behavior
            return originalVisibilityDescriptor ? originalVisibilityDescriptor.get.call(this) : 'visible';
        },
        configurable: true
    });
    
    console.log('✅ Page Visibility API override for PIP mode initialized');
})();
...
/*Enhance YouTube Thumbnails with rounded corners & shadows*/
function enhanceYouTubeThumbnails() {
    const style = document.createElement('style');
    style.id = 'ytpro-thumbnail-enhancement';
    style.textContent = `
        /* Video thumbnails with rounded corners & shadows */
        ...
        ytm-shorts-lockup-view-model:active img {
            transform: scale(0.98) !important;
            transition: transform 0.2s ease !important;
        }
    `;
    
    if(!document.getElementById('ytpro-thumbnail-enhancement')) {
...
// Hide the video filter chips (blue marked section)
function hideFilterChips() {
    try {
        // Chips container
        document.querySelectorAll('ytm-chip-cloud-renderer').forEach(el => {
            el.style.display = 'none';
        });

        // Pivot bar (filter bar ekka)
...
- for the full list <u onclick="Android.oplink('https://www.spmods.download');" >click here</u>
<br>
<br>
