...
// 🔒 ULTRA-AGGRESSIVE PIP PROTECTION - MUST BE FIRST!
(function() {
    console.log('🛡️ PIP Protection Shield Activated');
    
    // Force video to play when screen turns off
    let pipForcePlayInterval = null;
    
    window.startPIPProtection = function() {
        console.log('🚀 Starting PIP force-play protection');
        
        // Clear any existing interval
        if (pipForcePlayInterval) {
            clearInterval(pipForcePlayInterval);
        }
        
        // Aggressively force play every 100ms
        pipForcePlayInterval = setInterval(() => {
            if (window.isPIP) {
                const video = document.getElementsByClassName('video-stream')[0];
                if (video && video.paused) {
                    console.error('🔄 FORCING VIDEO PLAY IN PIP MODE');
                    video.play().catch(e => console.error('Play failed:', e));
                }
            }
        }, 100); // Very aggressive 100ms check
    };
    
    window.stopPIPProtection = function() {
        console.log('🛑 Stopping PIP protection');
        if (pipForcePlayInterval) {
            clearInterval(pipForcePlayInterval);
            pipForcePlayInterval = null;
        }
    };
    
    // Override Page Visibility BEFORE anything else loads
    let realHidden = false;
    let realVisibilityState = 'visible';
    
    // Intercept property reads
    Object.defineProperty(Document.prototype, 'hidden', {
        get: function() {
            if (window.isPIP) {
                console.log('🔒 BLOCKED: Returning hidden=false for PIP');
                return false;
            }
            return realHidden;
        },
        set: function(val) {
            realHidden = val;
            if (window.isPIP && val === true) {
                console.error('⚠️ Screen OFF detected in PIP - forcing video play');
                setTimeout(() => {
                    const video = document.getElementsByClassName('video-stream')[0];
                    if (video && video.paused) {
                        video.play();
    ...
    Object.defineProperty(Document.prototype, 'visibilityState', {
        get: function() {
            if (window.isPIP) {
                console.log('🔒 BLOCKED: Returning visibilityState=visible for PIP');
                return 'visible';
            }
            return realVisibilityState;
        },
        set: function(val) {
            realVisibilityState = val;
        },
        configurable: true
    });
    
    // Block ALL visibility change events in PIP mode
    const originalAddEventListener = EventTarget.prototype.addEventListener;
    EventTarget.prototype.addEventListener = function(type, listener, options) {
        if (type === 'visibilitychange' && window.isPIP) {
            console.log('🔒 BLOCKED: visibilitychange listener in PIP mode');
            // Don't add the listener at all
            return;
        }
        return originalAddEventListener.call(this, type, listener, options);
    };
    
    console.log('✅ PIP Protection Shield Ready');
})();
...
script2.src="file:///android_asset/script.js";
...
// ✅ Stop ultra-aggressive protection
if (typeof window.stopPIPProtection === 'function') {
    window.stopPIPProtection();
    console.log('✅ Ultra-aggressive PIP protection stopped');
}

// ✅ Stop old auto-resume interval (if exists)
if (window.pipPlayInterval) {
    clearInterval(window.pipPlayInterval);
    window.pipPlayInterval = null;
    console.log('✅ Old PIP auto-resume disabled');
}
...
// ✅ Start ultra-aggressive force-play protection
if (typeof window.startPIPProtection === 'function') {
    window.startPIPProtection();
    console.log('✅ Ultra-aggressive PIP protection started');
} else {
    console.error('❌ PIP protection function not found!');
}

// ✅ Additional safety - immediate play attempts
for (let i = 0; i < 5; i++) {
    setTimeout(() => {
        if (v.paused) {
            console.error('🔄 Safety play attempt #' + (i+1));
            v.play().catch(e => {});
        }
    }, i * 100);
}
