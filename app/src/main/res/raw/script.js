...
Fixed: inline onclick → data-action event delegation
*/


if(window.eruda == null && localStorage.getItem("devMode") == "true"){
window.location.href=`javascript:(function () { var script = document.createElement('script'); script.src="file:///android_asset/js/eruda.min.js"; document.body.appendChild(script); script.onload = function () { eruda.init();} })();`;
}
...
var GeminiAT="";
var GeminiModels={
"2.0 Flash":'[null,null,null,null,"f299729663a2343f"]',
"2.0 Flash Thinking": '[null,null,null,null,"7ca48d02d802f20a"]',
...
<path d="M16.59 9H15V4a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v5H7.41a1 1 0 0 0-.7 1.7l4.59 4.59a1 1 0 0 0 1.42 0l4.59-4.59a1 1 0 0 0-.72-1.7Z" stroke="${c}" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
...
function makeModifiedTypeChecker(origChecker) {
    return function (type) {
        ...
        if (localStorage['AV1'] === 'false') disallowed_types.push('av01', 'av99');
...
try{ referenceNode.parentNode.insertBefore(newNode, referenceNode.nextSibling); }catch{}
}
...
document.getElementById("hSett").addEventListener("click", function(ev){
...
try {
userLocales = new URL(Array.from(document.querySelectorAll("head > link[rel='search']"))?.find((n) => n?.getAttribute("href")?.includes("?locale="))?.getAttribute("href"))?.searchParams?.get("locale");
} catch { userLocales = "en"; }
}
...
var Url=new URL(url);
var vID="";
if(Url.pathname.indexOf("shorts") > -1){ vID=Url.pathname.substr(8,Url.pathname.length); }
else if(Url.pathname.indexOf("watch") > -1){ vID=Url.searchParams.get("v"); }
fetch("https://returnyoutubedislikeapi.com/votes?videoId="+vID)
.then(response => { return response.json(); })
.then(jsonObject => { if('dislikes' in jsonObject){ dislikes=getDislikesInLocale(parseInt(jsonObject.dislikes)); } })
.catch(error => {});
}
...
.then(response => { return response.json(); })
.then(jsonObject => { for(var x in jsonObject){ var time=jsonObject[x].segment; sTime.push(time); } })
.catch(error => {});
...
// FIX 1: addSkipper - inline onclick ඉවත් කර data-action delegate කරන්න
...
backdrop-filter:blur(6px);position:absolute;bottom:40px;line-height:50px;
left:calc(15% / 2);padding-left:10px;padding-right:10px;
z-index:99999999999999;text-align:center;border-radius:25px;color:white;text-align:center;
`);
...
<svg data-action="close" xmlns="http://www.w3.org/2000/svg" width="20" height="20" style="margin-left:30px;" fill="#f24" viewBox="0 0 16 16">
...
if(el.dataset.action == "close"){
    sSDiv.remove();
}else if(el.dataset.action == "rewind"){
    sSDiv.remove();
...
setTimeout(()=>{ try{sSDiv.remove();}catch{} },5000);
}
...
function sty(e,v){
var s={display:"flex",alignItems:"center",justifyContent:"center",fontWeight:"550",height:"65%",minWidth:"80px",width:"auto",borderRadius:"20px",background:d,fontSize:"12px",marginRight:"5px",textAlign:"center"};
for(x in s){ e.style[x]=s[x]; }
}


// FIX 2: getGeminiModels - data-action buttons
...
// FIX 3: getYTPROCodecs - data-action buttons
...
t+=`<br><br>
<div>Block 60FPS <span data-action="sttCnf" data-value="block_60fps" style="${sttCnf(0,0,"block_60fps")}" ><b style="${sttCnf(0,1,"block_60fps")}" ></b></span></div>`;
...
// FIX 4: ytproSettings - සියලු inline onclick → data-action
...
ytpSet.addEventListener("click", function(ev){
if(!(ev.target == ytpSetI || ytpSetI.contains(ev.target))){ history.back(); }
});

ytpSetI.setAttribute("style",`height:65%;width:calc(95% - 20px);overflow:auto;background:${isD ? "#212121" : "#f1f1f1"};position:fixed;bottom:20px;z-index:99999999999999;padding:10px;text-align:center;border-radius:25px;color:${isD ? "#ccc" : "#444"};`);

ytpSetI.innerHTML=`<style>
@import url('https://fonts.googleapis.com/css2?family=Delius&display=swap');
#settingsprodiv a{text-decoration:underline;}
#settingsprodiv li{list-style:none;display:flex;align-items:center;justify-content:center;color:#fff;border-radius:25px;padding:10px;background:#000;margin:5px;}
#ssprodivI div{height:10px;width:calc(100% - 20px);padding:10px;font-size:1.45rem;text-align:left;display:flex;align-items:center;position:relative;margin-top:3px;}
#ssprodivI div span{display:block;height:23px;width:40px;border-radius:40px;right:10px;position:absolute;background:#151515;}
#ssprodivI div span b{display:block;height:19px;width:19px;position:absolute;right:2px;top:2px;border-radius:50px;background:#fff;}
#ssprodivI div input::placeholder{color:${ isD ? "white" : "#000"};}
#ssprodivI div input,#ssprodivI div button{height:35px;background:${isD ? "rgba(255,255,255,.1)" : "rgba(0,0,0,.1)"};width:100%;border:0;border-radius:20px;padding:10px;font-size:1.25rem;}
#ssprodivI button{background:transparent;font-size:1.45rem;width:calc(100% - 20px);height:40px;color:${isD ? "#ccc" : "#444"};margin-top:3px;text-align:left;}
#ssprodivI button svg{float:right;}
#ssprodivI .credit{font-family:"Delius",cursive;font-style:normal;display:flex;justify-content:center;align-items:center;text-align:center;font-size:1.55rem;font-weight:bolder;color:${isD ? "#fff" : "#000"};position:fixed;bottom:20px;width:calc(95% - 20px);left:calc(2.5% + 0px);background:${d};border-radius:0 0 25px 25px;backdrop-filter:blur(10px);height:15px;}
#ssprodivI .geminiModels,#ssprodivI .disableCodecs,#ssprodivI .geminiPrompt{height:auto;min-height:100px;padding-bottom:12px;background:${isD ? "#212121" : "#f1f1f1"};position:fixed;display:none;width:calc(95% - 20px);left:calc(2.5% + 0px);bottom:20px;z-index:999999;box-shadow:0px 0px 5px black;border-radius:25px;}
#ssprodivI .geminiPrompt textarea{height:300px;width:95%;border-radius:20px;padding:15px;background:${d};}
#ssprodivI .disableCodecs{column:50%;}
#ssprodivI .disableCodecs button{width:48%;column:50%;margin-right:2%;color:${c};}
</style>`;

ytpSetI.innerHTML+=`<br><b style='font-size:18px'>SPYT PRO</b>
<span style="font-size:10px">config v${YTProVer}</span>
<br><br>
<div><input type="url" id="ytproUrlInput" placeholder="Enter Youtube URL"></div>
<br>
<button data-action="hearts">Liked Videos
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="${isD ? "#ccc" : "#444"}" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708"/></svg>
</button>
<br>
...
<br>
<button data-action="reportBug">Report Bugs
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="${isD ? "#ccc" : "#444"}" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708"/></svg>
</button>
<br>
<button style="font-weight:bolder;" data-action="telegram">Join Our Telegram Channel
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="${isD ? "#ccc" : "#444"}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 2l6 6-6 6"/></svg>
</button>
<br><br><br><br>
...
<path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314" stroke="black" ${ !isD ? "stroke-width='1'" : "" } stroke-linejoin="round" stroke-linecap="round"/>
</svg>
...
// FIX 5: searchUrl - proper event listener
document.getElementById("ytproUrlInput").addEventListener("keyup", function(e){
if(e.keyCode === 13){
var u=e.target.value;
var url="",id="";
try{ url=new URL(u); }catch{ Android.showToast("Enter a valid URL"); return; }
if((url.hostname.indexOf("youtube.com") > -1) || (url.hostname.indexOf("youtu.be") > -1)){
if(url.pathname.indexOf("shorts") > -1){ id=url.pathname.substr(8,url.pathname.length); }
else if(url.pathname.indexOf("watch") > -1){ id=url.searchParams.get("v"); }
else if(url.hostname.indexOf("youtu.be") > -1){ id=url.pathname.substr(1,url.pathname.length); }
}
...
reportBug: ()=>{ Android.oplink('mailto:spmodsofficial@gmail.com'); },
telegram: ()=>{ Android.oplink('https://t.me/SPModsSandun'); },
...
sttCnf: (el)=>{ sttCnf(el, el.dataset.value); },
setRemoveCodec: (el)=>{ setRemoveCodec(el.dataset.value, el); },
saveModel: (el)=>{
    localStorage.removeItem('geminiChatInfo');
    localStorage.setItem('geminiModel', el.dataset.value);
    el.parentElement.style.display='none';
}
};

// FIX 7: main settings panel delegation
ytpSetI.addEventListener("click",(e)=>{
var el=e.target.closest("[data-action]");
if(!el) return;
var action=el.dataset.action;
if(settingsActions[action]){
    settingsActions[action](el);
}
});

// FIX 8: disableCodecs panel delegation
ytpSetI.querySelector(".disableCodecs").addEventListener("click",(e)=>{
var el=e.target.closest("[data-action]");
if(!el) return;
if(settingsActions[el.dataset.action]){
    settingsActions[el.dataset.action](el);
}
});

// FIX 9: geminiModels panel delegation
...
// FIX 10: searchUrl global function (hash change ආදිය සඳහා)
...
if(isD){ var s=["#000","#717171","#fff"]; }
else{ var s=["#fff","#909090","#151515"]; }

if(typeof y == "string"){
if(localStorage.getItem(y) != "true"){
if(z == 1){ return `background:${s[0]};left:2px;`; }
else{ return `background:${s[1]};`; }
}else{
if(z == 1){ return `background:${s[0]};`; }
else{ return `background:${s[2]};`; }
}
}
...
try{ document.getElementById("brtS").remove(); document.getElementById("volS").remove(); }catch{}
}
...
ytproDown.setAttribute("style",`height:100%;width:100%;position:fixed;top:0;left:0;display:flex;justify-content:center;background:rgba(0,0,0,0.4);z-index:99999999999999;`);

ytproDown.addEventListener("click", function(ev){
if(ev.target != ytproDownDiv && !(ytproDownDiv.contains(ev.target))){ history.back(); }
});

ytproDownDiv.setAttribute("style",`height:50%;width:85%;overflow:auto;background:${isD ? "#212121" : "#f1f1f1"};position:absolute;bottom:20px;z-index:99999999999999;padding:20px;text-align:center;border-radius:25px;`);

document.body.appendChild(ytproDown);
ytproDown.appendChild(ytproDownDiv);

var id="";
if(window.location.pathname.indexOf("shorts") > -1){ id=window.location.pathname.substr(8,window.location.pathname.length); }
else{ id=new URLSearchParams(window.location.search).get("v"); }

ytproDownDiv.innerHTML="⏳ Loading...";

// All fetching + JSON parsing happens natively in Android/Java.
// Native calls back into window.ytproShowFormats(...) or window.ytproShowError(...) below.
Android.fetchVideoFormats(id);
}

window.ytproShowFormats=function(title, formatsJson, adaptiveJson, thumbJson){
var ytproDownDiv=document.getElementById("downytprodiv");
if(!ytproDownDiv) return;

var formats = JSON.parse(formatsJson || "[]");
var adaptiveFormats = JSON.parse(adaptiveJson || "[]");
var thumb = JSON.parse(thumbJson || "[]");
var safeTitle = (title || "video").replace(/[|\\?*<":>+\[\]/']/g,'_');

let html=`<style>
#downytprodiv h3{font-size:14px;margin-bottom:12px;color:${c};font-weight:600;}
#downytprodiv .section{margin:15px 0;text-align:left;}
#downytprodiv .section-title{font-size:13px;font-weight:700;color:${c};margin:10px 0 8px 0;padding:8px 10px;background:${isD ? 'rgba(66,133,244,0.15)' : 'rgba(6,95,212,0.1)'};border-left:3px solid ${isD ? '#4285f4' : '#065fd4'};border-radius:5px;}
#downytprodiv .dl-btn{display:block;width:100%;margin:5px 0;padding:10px;background:${d};border:0;border-radius:10px;text-align:left;color:${c};font-size:13px;}
</style><h3>${safeTitle}</h3>`;

if(formats.length > 0){
html+=`<div class="section"><div class="section-title">VIDEO + AUDIO</div>`;
formats.forEach(f=>{
const q=f.qualityLabel||f.quality||"?";
const s=f.contentLength?formatFileSize(f.contentLength):"";
html+=`<button class="dl-btn" data-url="${f.url}" data-name="${safeTitle}.mp4" data-type="video/mp4">📹 ${q} ${s}</button>`;
});
html+=`</div>`;
}
...
if(vids.length > 0){
html+=`<div class="section"><div class="section-title">VIDEO ONLY</div>`;
vids.slice(0,5).forEach(f=>{
const q=f.qualityLabel||(f.height+"p");
const fps=f.fps?` ${f.fps}fps`:"";
const s=f.contentLength?formatFileSize(f.contentLength):"";
html+=`<button class="dl-btn" data-url="${f.url}" data-name="${safeTitle}_${q}.mp4" data-type="video/mp4">🎬 ${q}${fps} ${s}</button>`;
});
html+=`</div>`;
}
...
if(auds.length > 0){
html+=`<div class="section"><div class="section-title">AUDIO ONLY</div>`;
auds.slice(0,3).forEach(f=>{
const br=f.bitrate?Math.round(f.bitrate/1000)+"kbps":"";
const s=f.contentLength?formatFileSize(f.contentLength):"";
html+=`<button class="dl-btn" data-url="${f.url}" data-name="${safeTitle}.m4a" data-type="audio/mp4">🎵 ${br} ${s}</button>`;
});
html+=`</div>`;
}

if(thumb && thumb.length > 0){
html+=`<div class="section"><div class="section-title">THUMBNAIL</div>`;
const t=thumb[thumb.length-1];
html+=`<button class="dl-btn" data-url="${t.url}" data-name="${safeTitle}_thumb.jpg" data-type="image/jpeg">📸 ${t.width} × ${t.height}</button>`;
html+=`</div>`;
}

ytproDownDiv.innerHTML=html;

ytproDownDiv.addEventListener("click",(e)=>{
var el=e.target.closest(".dl-btn");
if(!el) return;
Android.downvid(el.dataset.name, el.dataset.url, el.dataset.type);
Android.showToast('Download started!');
});
}

window.ytproShowError=function(message){
var ytproDownDiv=document.getElementById("downytprodiv");
if(!ytproDownDiv) return;
ytproDownDiv.innerHTML=`<div style="padding:20px;color:${c};"><h3>❌ Error</h3><p style="margin-top:10px;font-size:13px;">${message}</p><button data-action="close" style="margin-top:15px;padding:10px 20px;background:${c};color:${dc};border:0;border-radius:10px;">Close</button></div>`;
...
if(z > 1.05){ var Vv=document.getElementsByClassName('video-stream')[0]; zoomIn=true; scale=Math.max((screen.height/Vv.offsetHeight),(screen.width/Vv.offsetWidth)); addMaxButton(); }
else if(z < 0.95){ zoomIn=false; scale=1; addMaxButton(); }
}
}
},{capture:true});
...
if((e.target.className.toString().includes("video-stream")||e.target.className.toString().includes("player-controls-background"))&&!document.fullscreenElement&&localStorage.getItem("gesM")=="true"){ checkDirection(); }
if(e.touches.length < 2){ initialDistance=null; setTimeout(()=>{ stopProp=false; },500); }
},{capture:true});
...
if(e.destination.url.indexOf("/watch") > -1||e.destination.url.indexOf("/shorts") > -1){ window.location.href=e.destination.url; }
}else{ window.location.href=e.destination.url; }
});
return iframe;
};
...
startX=touch.clientX-rect.left; startY=touch.clientY-rect.top; isDragging=true;
...
element.addEventListener('touchend',function(){ isDragging=false; element.style.transition='all 0.3s ease'; },{passive:true});
}

function createCloseButton(){
var existingBtn=player.querySelector('.mini-close-btn');
if(existingBtn) return;
var closeBtn=document.createElement('button');
closeBtn.className='mini-close-btn';
closeBtn.innerHTML='×';
closeBtn.setAttribute('style','position:absolute;top:8px;right:8px;width:32px;height:32px;background:rgba(0,0,0,0.7);color:white;border:none;border-radius:50%;font-size:24px;line-height:28px;text-align:center;cursor:pointer;z-index:10000;padding:0;display:flex;align-items:center;justify-content:center;transition:all 0.2s ease;font-weight:300;');
closeBtn.addEventListener('click',function(e){ e.stopPropagation(); minimize(false); });
player.appendChild(closeBtn);
}

function removeCloseButton(){
var closeBtn=player.querySelector('.mini-close-btn');
...
player.setAttribute("ogLeft",getComputedStyle(player).left);
player.setAttribute("ogPosition",getComputedStyle(player).position);
player.setAttribute("ogWidth",getComputedStyle(player).width);
player.setAttribute("ogHeight",getComputedStyle(player).height);
player.setAttribute("ogTransform",getComputedStyle(player).transform);
player.setAttribute("ogBorderRadius",getComputedStyle(player).borderRadius);
player.setAttribute("ogZIndex",getComputedStyle(player).zIndex);
var miniWidth=window.innerWidth*0.45;
var miniHeight=miniWidth*(9/16);
player.style.display="block";
player.style.transition="all 0.3s ease";
player.style.transform="scale(1)";
player.style.position="fixed";
player.style.width=miniWidth+"px";
player.style.height=miniHeight+"px";
...
createCloseButton();
if(!player.hasAttribute("data-draggable")){ makeDraggable(player); player.setAttribute("data-draggable","true"); }
}else{
iframe.style.display="none";
player.style.transition="all 0.3s ease";
player.style.transform=player.getAttribute("ogTransform")||"scale(1)";
player.style.width=player.getAttribute("ogWidth");
player.style.height=player.getAttribute("ogHeight");
player.style.top=player.getAttribute("ogTop");
player.style.left=player.getAttribute("ogLeft");
player.style.bottom="auto";
player.style.right="auto";
player.style.position=player.getAttribute("ogPosition")||"relative";
player.style.zIndex=player.getAttribute("ogZIndex")||"auto";
player.style.borderRadius=player.getAttribute("ogBorderRadius")||"0px";
player.style.touchAction="auto";
player.style.boxShadow="none";
player.style.overflow="hidden";
removeCloseButton();
['ogTop','ogLeft','ogPosition','ogWidth','ogHeight','ogTransform','ogBorderRadius','ogZIndex','data-draggable'].forEach(a=>player.removeAttribute(a));
}
}


function callbackSNlM0e(){
return new Promise(resolve=>{ callbackSNlM0e.resolve=resolve; });
}

function callbackGeminiClient(){
return new Promise(resolve=>{ callbackGeminiClient.resolve=resolve; });
}
...
let thoughtsHtml=(thoughts!=null)?`<button onclick="(this.nextElementSibling.style.height=='auto')?(this.children[0].style.transform='rotate(-90deg)',this.nextElementSibling.style.height='0'):(this.children[0].style.transform='rotate(90deg)',this.nextElementSibling.style.height='auto');" class="think">Show Thinking<svg xmlns="http://www.w3.org/2000/svg" style="transform:rotate(-90deg);margin-left:10px" width="16" height="16" fill="${isD ? "#ccc" : "#444"}" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708"/></svg></button><div class="geminiThoughts"><br>${converter.makeHtml(thoughts)}</div><br>`:"";

document.getElementById("GeminiResponse").innerHTML=`<a href="https://gemini.google.com/chat/${chat[0].replace("c_","")}" >Go to the chat</a><br><br>${thoughtsHtml}<div class="geminiAnswer">${html}</div>`;
}
...
var secured="";
cookies.forEach((x)=>{ if(x.indexOf("__Secure-1PSID=") > -1||x.indexOf("__Secure-1PSIDTS=") > -1) secured+=x+";"; });

var endpoint="https://gemini.google.com/_/BardChatUi/data/assistant.lamda.BardFrontendService/StreamGenerate";
var headers=JSON.stringify({"accept":"*/*","accept-language":"en","content-type":"application/x-www-form-urlencoded;charset=UTF-8","x-goog-ext-525001261-jspb":GeminiModels[localStorage.getItem('geminiModel')],"x-same-domain":"1","cookie":secured,"Referer":"https://gemini.google.com/","Referrer-Policy":"origin"});

if(GeminiAT==""){
Android.getSNlM0e(secured);
GeminiAT=await callbackSNlM0e();
var sd=document.createElement('script');
sd.src="file:///android_asset/js/showdown.min.js";
...
var chat=null;
if(localStorage.getItem("saveCInfo")=="true"&&localStorage.getItem("geminiChatInfo")!=null){ chat=localStorage.getItem("geminiChatInfo").split(","); }
...
var elStyle={height:"100%",width:rect.width*0.15+"px",display:"flex","flex-direction":"column","align-items":"center","justify-content":"center",position:"absolute",top:"0px",right:"0px",opacity:"0"};
...
elB.addEventListener("touchmove",(e)=>{ e.preventDefault(); elB.style.opacity="1"; var diff=touchstartY-e.touches[0].pageY; if(diff>0){brt+=sens;}else{brt-=sens;} if(brt>1)brt=1; if(brt<0)brt=0; touchstartY=e.touches[0].pageY; Android.setBrightness(brt); document.getElementById("brtIS").style.height=brt*100+"%"; },{passive:false});
...
el.addEventListener("touchmove",(e)=>{ e.preventDefault(); el.style.opacity="1"; var diff=touchstartY-e.touches[0].pageY; if(diff>0){vol+=sens;}else{vol-=sens;} if(vol>1)vol=1; if(vol<0)vol=0; touchstartY=e.touches[0].pageY; Android.setVolume(vol); document.getElementById("volIS").style.height=vol*100+"%"; },{passive:false});
...
.geminiLoader,.geminiLoader:before,.geminiLoader:after{content:'';height:10px;width:70%;position:absolute;top:15px;border-radius:5px;left:10px;background:${d};animation:geminiLoad 1s linear infinite alternate;}
...
ytproGemini.addEventListener("click",async function(){
if(parseFloat(Android.getInfo()) < parseFloat(YTProVer)){ updateModel(); return; }
geminiInfo();
});

var ytproFavElem=document.createElement("div");
sty(ytproFavElem);
ytproFavElem.innerHTML=isHeart()
...
ysDown.setAttribute("style","height:48px;width:48px;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.3);border-radius:50%;margin-bottom:20px;backdrop-filter:blur(3px);");
...
ysHeart.setAttribute("style","height:48px;width:48px;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(3px);background:rgba(0,0,0,.3);border-radius:50%;margin-top:8px;margin-bottom:0px;");
ysHeart.innerHTML=isHeart()
...
ysHeart.addEventListener("click",function(){ ytProHeart(ysHeart); });
...
ytproH.setAttribute("style","height:100%;width:100%;position:fixed;top:0;left:0;display:flex;justify-content:center;background:rgba(0,0,0,0.4);z-index:99999999999999;");
ytproHh.setAttribute("style",`height:50%;width:85%;overflow:auto;background:${isD?"#212121":"#f1f1f1"};position:absolute;bottom:20px;z-index:99999999999999;padding:20px;text-align:center;border-radius:25px;`);
ytproHh.innerHTML=`<style>#heartytprodiv a{text-decoration:none;} #heartytprodiv li{list-style:none;display:flex;align-items:center;border-radius:15px;padding:0px;background:${isD?"rgba(0,0,0,.5)":"#fff"};margin:5px;} .thum{height:70px;border-radius:5px;} .thum img{float:left;height:70px;width:125px;border-radius:15px 0 0 15px;flex-shrink:0;}</style>`;
ytproHh.innerHTML+="Liked Videos<ul id='listurl'>";

document.body.appendChild(ytproH);
ytproH.appendChild(ytproHh);

ytproH.addEventListener("click",function(ev){
if(!event.composedPath().includes(ytproHh)){ history.back(); }
});
...
if(Object.keys(v).length===0){ return ytproHh.innerHTML+="No Videos Found"; }

for(var n=Object.keys(v).length-1;n>-1;n--){
var x=Object.keys(v)[n];
ytproHh.innerHTML+=`<li class="thum">
<img data-action="navigate" data-id="${x}" src="${v[x].thumb}">
<div style="width:calc(100% - 170px);margin-left:5px;overflow:hidden;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:3;overflow:hidden;text-overflow:ellipsis;" data-action="navigate" data-id="${x}">${v[x].title}</div>
<div style="width:calc(100% - (100% - 35px))">
<svg data-action="remHeart" data-id="${x}" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#f24" viewBox="0 0 16 16">
<path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z"/>
</svg>
</div>
</li>`;
...
// FIX: hearts list delegation
ytproHh.addEventListener("click",(e)=>{
var el=e.target.closest("[data-action]");
if(!el) return;
if(el.dataset.action=="navigate"){
window.location.href=`https://youtu.be/${el.dataset.id}`;
...
canvas.style.width="1600px"; canvas.style.height="900px"; canvas.style.background="black";
...
var vDetails;
if(window.location.pathname.indexOf("shorts") > -1){
vDetails={thumb:dataURI,title:document.getElementsByClassName('ytShortsVideoTitleViewModelShortsVideoTitle')[0].textContent.replaceAll("|","").replaceAll("\\","").replaceAll("?","").replaceAll("*","").replaceAll("<","").replaceAll("/","").replaceAll(":","").replaceAll('"',"").replaceAll(">","")};
}else{
vDetails={thumb:dataURI,title:document.getElementsByClassName('slim-video-metadata-header')[0].textContent.replaceAll("|","").replaceAll("\\","").replaceAll("?","").replaceAll("*","").replaceAll("<","").replaceAll("/","").replaceAll(":","").replaceAll('"',"").replaceAll(">","")};
}

var g="24";
var h=(window.location.href.indexOf('youtube.com/shorts') > -1)?``:`<span style="margin-left:8px">Heart</span>`;
var fillColor=(window.location.href.indexOf('youtube.com/shorts') > -1)?"#fff":c;

if(localStorage.getItem("hearts")?.indexOf(vid) > -1){
var j=JSON.parse(localStorage.getItem("hearts")||"{}"); delete j[vid]; localStorage.setItem("hearts",JSON.stringify(j));
x.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" width="${g}" height="${g}" fill="${fillColor}" viewBox="0 0 24 24"><path d="M0 0h24v24H0V0z" fill="none"/><path fill="${fillColor}" d="M19.66 3.99c-2.64-1.8-5.9-.96-7.66 1.1-1.76-2.06-5.02-2.91-7.66-1.1-1.4.96-2.28 2.58-2.34 4.29-.14 3.88 3.3 6.99 8.55 11.76l.1.09c.76.69 1.93.69 2.69-.01l.11-.1c5.25-4.76 8.68-7.87 8.55-11.75-.06-1.7-.94-3.32-2.34-4.28zM12.1 18.55l-.1.1-.1-.1C7.14 14.24 4 11.39 4 8.5 4 6.5 5.5 5 7.5 5c1.54 0 3.04.99 3.57 2.36h1.87C13.46 5.99 14.96 5 16.5 5c2 0 3.5 1.5 3.5 3.5 0 2.89-3.14 5.74-7.9 10.05z"/></svg>${h}`;
}else{
var j=JSON.parse(localStorage.getItem("hearts")||"{}"); j[vid]=vDetails; localStorage.setItem("hearts",JSON.stringify(j));
x.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" width="${g}" height="${g}" viewBox="0 0 24 24"><path d="M0 0h24v24H0V0z" fill="none"/><path fill="${fillColor}" d="M13.35 20.13c-.76.69-1.93.69-2.69-.01l-.11-.1C5.3 15.27 1.87 12.16 2 8.28c.06-1.7.93-3.33 2.34-4.29 2.64-1.8 5.9-.96 7.66 1.1 1.76-2.06 5.02-2.91 7.66-1.1 1.41.96 2.28 2.59 2.34 4.29.14 3.88-3.3 6.99-8.55 11.76l-.1.09z"/></svg>${h}`;
}
}
...
setTimeout(()=>{ document.getElementsByClassName('video-stream')[0].play(); },5);
}
...
v.requestFullscreen(); v.play(); pauseAllowed=false; isPIP=true;
setTimeout(()=>{ if(v.paused) v.play().catch(e=>{}); },50);
window.pipPlayInterval=setInterval(()=>{ if(window.isPIP&&v.paused) v.play().catch(e=>{}); },500);
}
...
let originalHiddenDescriptor=Object.getOwnPropertyDescriptor(Document.prototype,hiddenProp)||Object.getOwnPropertyDescriptor(document,hiddenProp);
Object.defineProperty(document,hiddenProp,{ get:function(){ if(window.isPIP) return false; return originalHiddenDescriptor?originalHiddenDescriptor.get.call(this):false; }, configurable:true });
let originalVisibilityDescriptor=Object.getOwnPropertyDescriptor(Document.prototype,'visibilityState')||Object.getOwnPropertyDescriptor(document,'visibilityState');
Object.defineProperty(document,'visibilityState',{ get:function(){ if(window.isPIP) return 'visible'; return originalVisibilityDescriptor?originalVisibilityDescriptor.get.call(this):'visible'; }, configurable:true });
})();
...
try{document.getElementById("settingsprodiv").remove();}catch{}
if(window.location.hash=="#download"){ if(typeof window.ytproSabrDownload === "function"){ window.ytproSabrDownload(); } else { ytproDownVid(); } }
else if(window.location.hash=="#settings"){ ytproSettings(); }
else if(window.location.hash=="#hearts"){ showHearts(); }
};
...
if(data?.responseContext?.webResponseContextExtensionData?.webResponseContextPreloadData?.preloadMessageNames?.[0]=="adSlotRenderer"||data?.responseContext?.webResponseContextExtensionData?.webResponseContextPreloadData?.preloadMessageNames?.[0]=="shortsAdsRenderer"){ data={}; }
delete data?.adSlots; delete data?.playerAds; delete data?.adPlacements; delete data?.adBreakHeartbeatParams;
delete data?.[0]?.playerResponse?.adSlots; delete data?.[0]?.playerResponse?.playerAds; delete data?.[0]?.playerResponse?.adPlacements; delete data?.[0]?.playerResponse?.adBreakHeartbeatParams;
...
this._interceptedMethod=method; this._interceptedUrl=url;
return origOpen.apply(this,[method,url,...rest]);
};

XHR.prototype.send=function(body){
if(this._interceptedUrl.includes("googleads.g.doubleclick.net")||this._interceptedUrl.includes("youtube.com/youtubei/v1/player/ad_break")||this._interceptedUrl.includes("youtube.com/pagead/adview")||this._interceptedUrl.includes("youtube.com/api/stats/ads")){ return; }
...
delete json.adSlots; delete json.playerAds; delete json.adPlacements; delete json.adBreakHeartbeatParams;
...
try{ document.getElementsByClassName("ad-interrupting")[0].getElementsByTagName("video")[0].currentTime=document.getElementsByClassName("ad-interrupting")[0].getElementsByTagName("video")[0].duration; document.getElementsByClassName("ytp-ad-skip-button-modern")[0].click(); }catch{}
...
try{ document.querySelectorAll('a').forEach(a=>{ if(a.href.indexOf("intent://") > -1) a.style.display='none'; }); }catch{}
...
try{ if(zoomIn){ Ve.style.transform=`scale(${scale})`; }else{ Ve.style.transform="scale(1)"; } }catch{}
}else{
try{ Ve.style.transform="scale(1)"; }catch{}
}
}

function hideFilterChips(){
try{ document.querySelectorAll('ytm-chip-cloud-renderer').forEach(el=>{ el.style.display='none'; }); }catch{}
try{ document.querySelectorAll('ytm-pivot-bar-renderer').forEach(el=>{ el.style.display='none'; }); }catch{}
}
...
x.innerHTML=`<div style="height:auto;width:70%;padding:20px;background:rgba(0,0,0,.6);border:1px solid #888;box-shadow:0px 0px 5px black;color:white;backdrop-filter:blur(10px);border-radius:15px;margin:auto">
<h2>Mandatory Update</h2><br>
Latest Version ${YTProVer} of YTPRO is available.<br><br>
<div style="display:flex;">
<button data-action="download" style="border:0;border-radius:10px;height:30px;width:150px;background:rgba(255,50,50,.7);float:right;">Download</button>
</div></div>`;
x.addEventListener("click",(e)=>{
var el=e.target.closest("[data-action]");
if(!el) return;
if(el.dataset.action=="download"){ Android.downvid('YTPRO.zip','https://www.spmods.download','application/zip'); }
});
...
if(parseFloat(Android.getInfo()) < parseFloat(YTProVer)&&(window.location.href=="https://m.youtube.com/"||window.location.href=="https://m.youtube.com")){ updateModel(); }
};
...
if(!pc) return;
originalPlayerContainer={position:pc.style.position,top:pc.style.top,left:pc.style.left,width:pc.style.width,height:pc.style.height,zIndex:pc.style.zIndex,transform:pc.style.transform};
pc.style.position='fixed'; pc.style.bottom='70px'; pc.style.left='10px'; pc.style.width='45%'; pc.style.height='auto'; pc.style.zIndex='9999'; pc.style.borderRadius='12px'; pc.style.boxShadow='0 4px 12px rgba(0,0,0,0.5)'; pc.style.top='auto'; pc.style.right='auto';
...
pc.onclick=(e)=>{ if(e.target!==cb&&!cb.contains(e.target)){ closeMiniPlayer(); window.scrollTo({top:0,behavior:'smooth'}); } };
pc.appendChild(cb);
...
if(cb) cb.remove();
Object.keys(originalPlayerContainer).forEach(k=>{ pc.style[k]=originalPlayerContainer[k]; });
pc.style.bottom=''; pc.style.right=''; pc.style.borderRadius=''; pc.style.boxShadow=''; pc.onclick=null;
